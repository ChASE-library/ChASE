...
TYPED_TEST(MultiVectorDistTest, RowCommInternalMemoryAllocation)
{
    ...
    // using SinglePrecisionType = typename
    // chase::ToSinglePrecisionTrait<T>::Type;
    ...
    std::size_t M = 6;
    std::size_t N = 4;
    auto multivector_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::row>(M, N, mpi_grid);
...
TYPED_TEST(MultiVectorDistTest, ColumnCommInternalMemoryAllocation)
{
    ...
    std::size_t N = 4;
    auto multivector_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column>(M, N, mpi_grid);
...
TYPED_TEST(MultiVectorDistTest, RowCommExternalMemoryAllocation)
{
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVector1D<
        ...
        lrows, lcols, lld, buffer.data(), mpi_grid);
...
TYPED_TEST(MultiVectorDistTest, RedistributionFromColumnToRowCommunicator)
{
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column>(M, N, mpi_grid);
    auto target_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::row>(M, N, mpi_grid);
    ...
    for (auto i = 0; i < multivector_.l_cols(); i++)
    {
        ...
        {
            multivector_.l_data()[i * multivector_.l_ld() + j] =
                T(coords[0] + i + j);
        }
    }
    ...
    for (auto i = 0; i < target_.l_cols(); i++)
    {
        for (auto j = 0; j < target_.l_rows(); j++)
        {
            ...
            {
                EXPECT_EQ(target_.l_data()[i * target_.l_ld() + j],
                          T(coords[1] + i + j));
            }
            else
            {
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromColumnToRowCommunicatorNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromRowToColumnCommunicatorNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest, RedistributionFromSameCommunicatorExpectFail)
{
    ...
    EXPECT_THROW(
        { multivector_.redistributeImpl(&target_); }, std::runtime_error);

    auto multivector_2 = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column>(M, N, mpi_grid);
    auto target_2 = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column>(M, N, mpi_grid);

    EXPECT_THROW(
        { multivector_2.redistributeImpl(&target_2); }, std::runtime_error);

    EXPECT_THROW(
        { multivector_.redistributeImpl(&target_2, 10, 10); },
        std::invalid_argument);
    EXPECT_THROW(
        { multivector_2.redistributeImpl(&target_, 10, 10); },
        std::invalid_argument);
}
...
TYPED_TEST(MultiVectorDistTest, BlockCyclicRowCommInternalMemoryAllocation)
{
    ...
    std::size_t N = 4;
    std::size_t mb = 2;
    auto multivector_ = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::row>(M, N, mb, mpi_grid);
...
TYPED_TEST(MultiVectorDistTest, BlockCyclicColumnCommInternalMemoryAllocation)
{
    ...
    std::size_t mb = 2;
    auto multivector_ = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::column>(M, N, mb,
                                                             mpi_grid);
...
TYPED_TEST(MultiVectorDistTest, BlockCyclicRowCommExternalMemoryAllocation)
{
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::row>(
        M, m, N, mb, lld, buffer.data(), mpi_grid);
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromColumnToRowCommunicator)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::column>(M, N, mb,
                                                             mpi_grid);
    auto target_ = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::row>(M, N, mb, mpi_grid);
    int* coords = mpi_grid.get()->get_coords();
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromRowToColumnCommunicator)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromColumnToRowCommunicatorNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromRowToColumnCommunicatorNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest, Block1DMixedPrecison)
{
    ...
    std::size_t grows = 6;
    std::size_t gcols = 4;
    auto block1Dmultivector_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column>(grows, gcols,
                                                             mpi_grid);
    ...
    for (auto i = 0; i < block1Dmultivector_.l_cols(); i++)
    {
        ...
        {
            block1Dmultivector_.l_data()[i * block1Dmultivector_.l_ld() + j] =
                T(i + j);
        }
    }
                  ...
                  std::is_same<T, std::complex<double>>::value)
    {
        ...
        auto* single_precision_multivec =
            block1Dmultivector_.getSinglePrecisionMatrix();
        ...
        for (auto i = 0; i < single_precision_multivec->l_cols(); i++)
        {
            ...
            {
                EXPECT_EQ(
                    single_precision_multivec
                        ->l_data()[i * single_precision_multivec->l_ld() + j],
                    SinglePrecisionType(i + j));
                single_precision_multivec
                    ->l_data()[i * single_precision_multivec->l_ld() + j] +=
                    SinglePrecisionType(0.5);
            }
        }
        ...
        for (auto i = 0; i < block1Dmultivector_.l_cols(); i++)
        {
            for (auto j = 0; j < block1Dmultivector_.l_rows(); j++)
            {
                EXPECT_EQ(block1Dmultivector_
                              .l_data()[i * block1Dmultivector_.l_ld() + j],
                          T(i + j + 0.5));
            }
        }
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromColumnToRowCommunicatorGPUCUDAAwareMPI)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column,
        chase::platform::GPU>(lrows, N, lrows, src_data.data(), mpi_grid);
    auto target_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::row, chase::platform::GPU>(
        lrows, N, lrows, dest_data.data(), mpi_grid);
    ...
    for (auto i = 0; i < target_.l_cols(); i++)
    {
        for (auto j = 0; j < target_.l_rows(); j++)
        {
            if (i >= offset && i < offset + subSize)
            {
                EXPECT_EQ(target_.cpu_data()[i * target_.cpu_ld() + j],
                          T(coords[1] + i + j));
            }
            else
            {
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromRowToColumnCommunicatorGPUCUDAAwareMPI)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::row, chase::platform::GPU>(
        lrows, N, lrows, src_data.data(), mpi_grid);
    auto target_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column,
        chase::platform::GPU>(lrows, N, lrows, dest_data.data(), mpi_grid);
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromColumnToRowCommunicatorGPUNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromRowToColumnCommunicatorGPUNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicColumnCommInternalMemoryAllocationGPU)
{
    using T = TypeParam; // Get the current type
    // using SinglePrecisionType = typename
    // chase::ToSinglePrecisionTrait<T>::Type;
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::column,
        chase::platform::GPU>(M, N, mb, mpi_grid);
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicColumnCommExternalMemoryAllocationGPU)
{
    using T = TypeParam; // Get the current type
    // using SinglePrecisionType = typename
    // chase::ToSinglePrecisionTrait<T>::Type;
    ...
    auto multivector_ = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::column,
        chase::platform::GPU>(M, m, N, mb, lld, buffer.data(), mpi_grid);
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromColumnToRowCommunicatorGPU)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
    ...
    for (auto i = 0; i < multivector_.l_cols(); i++)
    {
        for (auto j = 0; j < multivector_.l_rows(); j++)
        {
            multivector_.cpu_data()[i * multivector_.cpu_ld() + j] =
                T(coords[0] + i + j);
        }
    }
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromRowToColumnCommunicatorGPU)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(
    MultiVectorDistTest,
    BlockCyclicRedistributionFromColumnToRowCommunicatorGPUNonsquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(
    MultiVectorDistTest,
    BlockCyclicRedistributionFromRowToColumnCommunicatorGPUNonsquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
#ifdef HAS_NCCL
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromColumnToRowCommunicatorGPUNCCL)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionFromColumnToRowCommunicatorGPUNCCLNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionAsyncFromRowToColumnCommunicatorGPUNCCL)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           RedistributionAsyncFromRowToColumnCommunicatorGPUNCCLNonSquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromColumnToRowCommunicatorGPUNCCL)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(MultiVectorDistTest,
           BlockCyclicRedistributionFromRowToColumnCommunicatorGPUNCCL)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(
    MultiVectorDistTest,
    BlockCyclicRedistributionFromColumnToRowCommunicatorGPUNCCLNonsquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
...
TYPED_TEST(
    MultiVectorDistTest,
    BlockCyclicRedistributionFromRowToColumnCommunicatorGPUNCCLNonsquaredGrid)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
