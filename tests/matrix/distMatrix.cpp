...
TYPED_TEST(MatrixDistTest, RedundantInternalMemoryAllocation)
{
    ...
    std::size_t M = 5;
    std::size_t N = 4;
    auto redundant_matrix_ =
        chase::distMatrix::RedundantMatrix<T>(M, N, mpi_grid);
...
TYPED_TEST(MatrixDistTest, RedundantExternalMemoryAllocation)
{
    ...
    auto redundant_matrix_ = chase::distMatrix::RedundantMatrix<T>(
        M, N, ld, buffer.data(), mpi_grid);
...
TYPED_TEST(MatrixDistTest, BlockBlockInternalMemoryAllocation)
{
    ...
    std::size_t M = 6;
    std::size_t N = 6;
    auto blockblockmatrix_ =
        chase::distMatrix::BlockBlockMatrix<T>(M, N, mpi_grid);
...
TYPED_TEST(MatrixDistTest, BlockBlockExternalMemoryAllocation)
{
    ...
    auto blockblockmatrix_ = chase::distMatrix::BlockBlockMatrix<T>(
        l_rows, l_cols, l_ld, buffer.data(), mpi_grid);
    ...
    for (auto i = 0; i < blockblockmatrix_.l_cols(); i++)
    {
        ...
        {
            EXPECT_EQ(
                blockblockmatrix_.l_data()[i * blockblockmatrix_.l_ld() + j],
                T(i + j));
        }
    }
}
...
TYPED_TEST(MatrixDistTest, BlockBlockRedistributionToRedundantDivisibleSize)
{
    ...
    auto blockblockmatrix_ =
        chase::distMatrix::BlockBlockMatrix<T>(g_rows, g_cols, mpi_grid);
    auto redundantmatrix_ =
        chase::distMatrix::RedundantMatrix<T>(g_rows, g_cols, mpi_grid);
    ...
    for (auto i = 0; i < redundantmatrix_.l_rows(); i++)
    {
        ...
        {
            EXPECT_EQ(
                redundantmatrix_.l_data()[i + j * redundantmatrix_.l_ld()],
                T(i + j * redundantmatrix_.l_ld()));
        }
    }
    // test also from redundant to blockblock
    auto blockblockmatrix_2 =
        chase::distMatrix::BlockBlockMatrix<T>(g_rows, g_cols, mpi_grid);
    ...
    for (auto i = 0; i < blockblockmatrix_2.l_rows(); i++)
    {
        ...
        {
            EXPECT_EQ(
                blockblockmatrix_2.l_data()[i + j * blockblockmatrix_2.l_ld()],
                blockblockmatrix_.l_data()[i + j * blockblockmatrix_.l_ld()]);
        }
    }
}
...
TYPED_TEST(MatrixDistTest, BlockBlockRedistributionToRedundantIndivisibleSize)
{
    ...
    startCol = 3;
    subCols = 1;
    redundantmatrix_.redistributeImpl(&blockblockmatrix_2, startRow, subRows,
                                      startCol, subCols);
    ...
    for (auto i = 0; i < blockblockmatrix_2.l_rows(); i++)
    {
        for (auto j = 0; j < blockblockmatrix_2.l_cols(); j++)
        {
            ...
            {
                EXPECT_EQ(blockblockmatrix_2
                              .l_data()[i + j * blockblockmatrix_2.l_ld()],
                          blockblockmatrix_
                              .l_data()[i + j * blockblockmatrix_.l_ld()]);
            }
            else
            {
                EXPECT_EQ(blockblockmatrix_2
...
TYPED_TEST(MatrixDistTest, BlockCyclicExternalMemoryAllocationCPU)
{
    ...
    auto blockcyclicmatrix_ =
        chase::distMatrix::BlockCyclicMatrix<T, chase::platform::CPU>(
            M, N, m, n, mb, nb, ld, data.data(), mpi_grid);
...
TYPED_TEST(MatrixDistTest, RedundantRedistributionToBlockCyclic)
{
    ...
    auto redundantmatrix_ =
        chase::distMatrix::RedundantMatrix<T, chase::platform::CPU>(M, N,
                                                                    mpi_grid);
    auto blockcyclicmatrix_ =
        chase::distMatrix::BlockCyclicMatrix<T, chase::platform::CPU>(
            M, N, blocksize, blocksize, mpi_grid);

    for (auto i = 0; i < redundantmatrix_.l_rows() * redundantmatrix_.l_cols();
         i++)
    {
...
TYPED_TEST(MatrixDistTest, BlockCyclicRedistributionToRedundant)
{
    using T = TypeParam; // Get the current type
    // using SinglePrecisionType = typename
    // chase::ToSinglePrecisionTrait<T>::Type;
    ...
    for (auto i = 0; i < redundantmatrix_.l_rows() * redundantmatrix_.l_cols();
         i++)
    {
        redundantmatrix_.l_data()[i] = T(-1);
    }

    if (this->world_rank == 0)
    {
        blockcyclicmatrix_.l_data()[0] = T(0.0);
        blockcyclicmatrix_.l_data()[1] = T(2.0);
        blockcyclicmatrix_.l_data()[2] = T(4.0);
        blockcyclicmatrix_.l_data()[3] = T(10.0);
        blockcyclicmatrix_.l_data()[4] = T(12.0);
        blockcyclicmatrix_.l_data()[5] = T(14.0);
        blockcyclicmatrix_.l_data()[6] = T(20.0);
        blockcyclicmatrix_.l_data()[7] = T(22.0);
        blockcyclicmatrix_.l_data()[8] = T(24.0);
    }
    else if (this->world_rank == 1)
    {
        blockcyclicmatrix_.l_data()[0] = T(1.0);
        blockcyclicmatrix_.l_data()[1] = T(3.0);
        blockcyclicmatrix_.l_data()[2] = T(11.0);
        blockcyclicmatrix_.l_data()[3] = T(13.0);
        blockcyclicmatrix_.l_data()[4] = T(21.0);
        blockcyclicmatrix_.l_data()[5] = T(23.0);
    }
    else if (this->world_rank == 2)
    {
        blockcyclicmatrix_.l_data()[0] = T(5.0);
        blockcyclicmatrix_.l_data()[1] = T(7.0);
        blockcyclicmatrix_.l_data()[2] = T(9.0);
        blockcyclicmatrix_.l_data()[3] = T(15.0);
        blockcyclicmatrix_.l_data()[4] = T(17.0);
        blockcyclicmatrix_.l_data()[5] = T(19.0);
    }
    else
    {
        blockcyclicmatrix_.l_data()[0] = T(6.0);
        blockcyclicmatrix_.l_data()[1] = T(8.0);
        blockcyclicmatrix_.l_data()[2] = T(16.0);
        blockcyclicmatrix_.l_data()[3] = T(18.0);
    }
    ...
    for (auto i = 0; i < redundantmatrix_.l_rows() * redundantmatrix_.l_cols();
         i++)
    {
        EXPECT_EQ(redundantmatrix_.l_data()[i], T(i));
    }
}
...
TYPED_TEST(MatrixDistTest, RedundantInternalMemoryAllocationGPU)
{
    ...
    std::size_t N = 4;
    auto redundant_matrix_ =
        chase::distMatrix::RedundantMatrix<T, chase::platform::GPU>(M, N,
                                                                    mpi_grid);
...
TYPED_TEST(MatrixDistTest, BlockBlockInternalMemoryAllocationGPU)
{
    ...
    std::size_t N = 6;
    auto blockblockmatrix_ =
        chase::distMatrix::BlockBlockMatrix<T, chase::platform::GPU>(M, N,
                                                                     mpi_grid);
...
TYPED_TEST(MatrixDistTest, BlockCyclicRedistributionToRedundantGPUNCCL)
{
    ...
    blockcyclicmatrix_.redistributeImplAsync(&redundantmatrix_);
