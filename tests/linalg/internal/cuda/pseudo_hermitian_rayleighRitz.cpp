...
protected:
    void SetUp() override
    {
        ...
        H = new chase::matrix::PseudoHermitianMatrix<T, chase::platform::GPU>(
            N, N);
        H->allocate_cpu_data();
        Q = chase::matrix::PseudoHermitianMatrix<T, chase::platform::GPU>(
            N, nev, N, Q_buffer.data());
        V = chase::matrix::PseudoHermitianMatrix<T, chase::platform::GPU>(N,
                                                                          nev);
        ritzv =
            chase::matrix::Matrix<chase::Base<T>, chase::platform::GPU>(nev, 1);
        ...
        Q_tiny = chase::matrix::PseudoHermitianMatrix<T, chase::platform::GPU>(
            N_tiny, nev_tiny, N_tiny, Q_buffer_tiny.data());
        V_tiny = chase::matrix::PseudoHermitianMatrix<T, chase::platform::GPU>(
            N_tiny, nev_tiny);
        ritzv_tiny =
            chase::matrix::Matrix<chase::Base<T>, chase::platform::GPU>(nev, 1);
        exact_eigsl_H_tiny =
            chase::matrix::Matrix<T, chase::platform::CPU>(N_tiny, 1);
...
TYPED_TEST(PseudoHermitianRayleighRitzGPUTest, PseudoHermitianRayleighRitz)
{
    ...
    chase::linalg::internal::cuda::rayleighRitz(
        this->cublasH_, this->cusolverH_, this->params_, this->H, this->Q,
        this->V, this->ritzv, (std::size_t)0, this->nev, &info);
    ...
    {
        std::cout << this->ritzv.cpu_data()[i] << "vs."
                  << this->exact_eigsl_H.data()[i] << std::endl;
        EXPECT_LT(this->ritzv.cpu_data()[i],
                  std::real(this->exact_eigsl_H.data()[i]) +
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
        EXPECT_GT(this->ritzv.cpu_data()[i],
                  std::real(this->exact_eigsl_H.data()[i]) -
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
    }
...
TYPED_TEST(PseudoHermitianRayleighRitzGPUTest, tinyPseudoHermitianRayleighRitz)
{
    ...
    chase::linalg::internal::cuda::rayleighRitz(
        this->cublasH_, this->cusolverH_, this->params_, this->H_tiny,
        this->Q_tiny, this->V_tiny, this->ritzv_tiny, (std::size_t)0,
        this->nev_tiny, &info);
    ...
    {
        std::cout << this->ritzv_tiny.cpu_data()[i] << "vs."
                  << this->exact_eigsl_H_tiny.data()[i] << std::endl;
        EXPECT_LT(this->ritzv_tiny.cpu_data()[i],
                  std::real(this->exact_eigsl_H_tiny.data()[i]) +
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
        EXPECT_GT(this->ritzv_tiny.cpu_data()[i],
                  std::real(this->exact_eigsl_H_tiny.data()[i]) -
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
    }
