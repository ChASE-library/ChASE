...
TYPED_TEST(rayleighRitzGPUTest, eigenpairs)
{
    ...
    chase::matrix::Matrix<T, chase::platform::GPU>* Hmat =
        new chase::matrix::Matrix<T, chase::platform::GPU>(this->N, this->N,
                                                           this->N, H2.data());
    ...
    chase::matrix::Matrix<chase::Base<T>, chase::platform::GPU> ritzv(
        this->n, 1, this->n, ritzv_data.data());

    CHECK_CUDA_ERROR(cudaMemcpy(V1.data(), this->H.data(),
                                this->N * this->n * sizeof(T),
                                cudaMemcpyHostToDevice));
    CHECK_CUDA_ERROR(cudaMemcpy(ritzv.data(), this->evals.data(),
    ...
    chase::linalg::internal::cuda::rayleighRitz(
        this->cublasH_, this->cusolverH_, Hmat, V1, V2, ritzv, offset, subSize,
        devInfo);
