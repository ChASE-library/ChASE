...
TYPED_TEST(lacpyGPUTest, fullCopy)
{
    ...
    chase::matrix::Matrix<T, chase::platform::GPU> src_matrix(rows, cols, ld,
                                                              buffer.data());
    chase::matrix::Matrix<T, chase::platform::GPU> target_matrix(rows, cols);
    chase::linalg::internal::cuda::t_lacpy(
        'A', rows, cols, src_matrix.data(), src_matrix.ld(),
        target_matrix.data(), target_matrix.ld());
    target_matrix.D2H();
    ...
    for (auto j = 0; j < cols; j++)
    {
        ...
        {
            EXPECT_EQ(target_matrix.cpu_data()[j * rows + i],
                      buffer.data()[j * ld + i]);
        }
    }
}
...
TYPED_TEST(lacpyGPUTest, upperCopy)
{
    ...
    chase::matrix::Matrix<T, chase::platform::GPU> src_matrix(rows, cols, rows,
                                                              buffer.data());
    chase::matrix::Matrix<T, chase::platform::GPU> target_matrix(
        rows, cols, rows, buffer2.data());
    chase::linalg::internal::cuda::t_lacpy(
        'U', rows, cols, src_matrix.data(), src_matrix.ld(),
        target_matrix.data(), target_matrix.ld());
    target_matrix.D2H();
...
TYPED_TEST(lacpyGPUTest, lowerCopy)
{
    ...
    chase::matrix::Matrix<T, chase::platform::GPU> target_matrix(
        rows, cols, rows, buffer2.data());
    chase::linalg::internal::cuda::t_lacpy(
        'L', rows, cols, src_matrix.data(), src_matrix.ld(),
        target_matrix.data(), target_matrix.ld());
    target_matrix.D2H();
