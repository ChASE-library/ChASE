...
template <typename T>
class PseudoRayleighRitzGPUDistTest : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
TYPED_TEST_SUITE(PseudoRayleighRitzGPUDistTest, TestTypes);

TYPED_TEST(PseudoRayleighRitzGPUDistTest,
           TinyPseudoHermitianRRDistGPUCorrectness)
{
    ...
    chase::linalg::internal::cuda_mpi::pseudo_hermitian_rayleighRitz(
...
TYPED_TEST(PseudoRayleighRitzGPUDistTest, PseudoHermitianRRDistGPUCorrectness)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
