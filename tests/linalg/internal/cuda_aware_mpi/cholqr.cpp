    ...
    void SetUp() override
    {
        ...
        if (d_V == nullptr || d_V_size < required_size)
        {
            ...
            CHECK_CUDA_ERROR(cudaMalloc(
                (void**)&d_V, sizeof(std::complex<double>) * required_size));
...
TYPED_TEST(CHOLQRCUDAAwareMPIDistTest, cholQR1GPU)
{
    ...
    read_vectors(V.data(), GetQRFileName<T>() + "cond_10.bin", xoff, xlen,
                 this->N, this->n, 0);
    CHECK_CUDA_ERROR(
        cudaMemcpy((reinterpret_cast<T*>(this->get_device_vector())), V.data(),
                   sizeof(T) * xlen * this->n, cudaMemcpyHostToDevice));
    int info = chase::linalg::internal::cuda_mpi::cholQR1<T>(
        this->get_cublas_handle(), this->get_cusolver_handle(), xlen, this->n,
        reinterpret_cast<T*>(this->get_device_vector()), xlen,
        this->get_mpi_grid()->get_comm());
    ASSERT_EQ(info, 0);
    CHECK_CUDA_ERROR(
        cudaMemcpy(V.data(), reinterpret_cast<T*>(this->get_device_vector()),
                   sizeof(T) * xlen * this->n, cudaMemcpyDeviceToHost));
...
TYPED_TEST(CHOLQRCUDAAwareMPIDistTest, cholQR1BadlyCondGPU)
{
    ...
    std::size_t xoff = this->get_world_rank() * 25;
    std::vector<T> V(xlen * this->n);
    read_vectors(V.data(), GetQRFileName<T>() + "cond_1e4.bin", xoff, xlen,
                 this->N, this->n, 0);
    CHECK_CUDA_ERROR(
        cudaMemcpy((reinterpret_cast<T*>(this->get_device_vector())), V.data(),
                   sizeof(T) * xlen * this->n, cudaMemcpyHostToDevice));
...
TYPED_TEST(CHOLQRCUDAAwareMPIDistTest, cholQR1illCondGPU)
{
    ...
    std::size_t xoff = this->get_world_rank() * 25;
    std::vector<T> V(xlen * this->n);
    read_vectors(V.data(), GetQRFileName<T>() + "cond_ill.bin", xoff, xlen,
                 this->N, this->n, 0);
    CHECK_CUDA_ERROR(
        cudaMemcpy((reinterpret_cast<T*>(this->get_device_vector())), V.data(),
                   sizeof(T) * xlen * this->n, cudaMemcpyHostToDevice));
...
TYPED_TEST(CHOLQRCUDAAwareMPIDistTest, cholQR2GPU)
{
    ...
    CHECK_CUDA_ERROR(
        cudaMemcpy((reinterpret_cast<T*>(this->get_device_vector())), V.data(),
                   sizeof(T) * xlen * this->n, cudaMemcpyHostToDevice));
    int info = chase::linalg::internal::cuda_mpi::cholQR2<T>(
        this->get_cublas_handle(), this->get_cusolver_handle(), xlen, this->n,
        reinterpret_cast<T*>(this->get_device_vector()), xlen,
        this->get_mpi_grid()->get_comm());
    ASSERT_EQ(info, 0);
...
TYPED_TEST(CHOLQRCUDAAwareMPIDistTest, scholQRGPU)
{
    ...
    CHECK_CUDA_ERROR(
        cudaMemcpy((reinterpret_cast<T*>(this->get_device_vector())), V.data(),
                   sizeof(T) * xlen * this->n, cudaMemcpyHostToDevice));
    int info = chase::linalg::internal::cuda_mpi::shiftedcholQR2<T>(
