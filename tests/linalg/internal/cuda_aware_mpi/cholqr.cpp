...
template <typename T>
class CHOLQRGPUDistTest : public ::testing::Test {
protected:
    void SetUp() override {
...
TYPED_TEST_SUITE(CHOLQRGPUDistTest, TestTypes);

TYPED_TEST(CHOLQRGPUDistTest, cholQR1GPU) {
...
TYPED_TEST(CHOLQRGPUDistTest, cholQR1BadlyCondGPU) {
    using T = TypeParam;  // Get the current type
    assert(this->world_size == 4);
    auto machineEpsilon = MachineEpsilon<T>::value();
    std::size_t xlen = this->N / this->world_size;
...
TYPED_TEST(CHOLQRGPUDistTest, cholQR1illCondGPU) {
    using T = TypeParam;  // Get the current type
    assert(this->world_size == 4);
    auto machineEpsilon = MachineEpsilon<T>::value();
    std::size_t xlen = this->N / this->world_size;
...
TYPED_TEST(CHOLQRGPUDistTest, cholQR2GPU) {
    using T = TypeParam;  // Get the current type
    assert(this->world_size == 4);
    auto machineEpsilon = MachineEpsilon<T>::value();
    std::size_t xlen = this->N / this->world_size;
...
TYPED_TEST(CHOLQRGPUDistTest, cholQR2IllCondGPU) {
    using T = TypeParam;  // Get the current type
    assert(this->world_size == 4);
    auto machineEpsilon = MachineEpsilon<T>::value();
    std::size_t xlen = this->N / this->world_size;
...
TYPED_TEST(CHOLQRGPUDistTest, scholQRGPU) {
    using T = TypeParam;  // Get the current type
    assert(this->world_size == 4);
    auto machineEpsilon = MachineEpsilon<T>::value();
    std::size_t xlen = this->N / this->world_size;
...
/*
TYPED_TEST(CHOLQRGPUDistTest, modifiedGramSchmidtCholQRGPU) {
    using T = TypeParam;  // Get the current type
    assert(this->world_size == 4);
    auto machineEpsilon = MachineEpsilon<T>::value();
    std::size_t xlen = this->N / this->world_size;
    ...
    ASSERT_NEAR(orth, machineEpsilon, machineEpsilon * 1e4);
    cudaFree(d_V);
}
*/