...
template <typename T>
class SymOrHermGPUCUDAAwareMPI : public ::testing::Test {
protected:
    void SetUp() override {
...
TYPED_TEST_SUITE(SymOrHermGPUCUDAAwareMPI, TestTypes);

TYPED_TEST(SymOrHermGPUCUDAAwareMPI, UpperTriangularMatrix) {
    using T = TypeParam;
    ...
    H.H2D();
    bool is_sym = chase::linalg::internal::cuda_mpi::checkSymmetryEasy(cublasH, H); 
    EXPECT_FALSE(is_sym);
#ifdef HAS_SCALAPACK
    chase::linalg::internal::cuda_mpi::symOrHermMatrix('U', H);
    H.H2D();
    is_sym = chase::linalg::internal::cuda_mpi::checkSymmetryEasy(cublasH, H);    
    EXPECT_TRUE(is_sym);
#endif    
}

TYPED_TEST(SymOrHermGPUCUDAAwareMPI, LowerTriangularMatrix) {
    using T = TypeParam;
    const std::size_t N = 5;
