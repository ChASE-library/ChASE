...
protected:
    static void SetUpTestSuite()
    {
        ...
        if (!resources_initialized)
        {
            ...
            CHECK_CUDA_ERROR(
                cudaMalloc(&H_buff, m * m * sizeof(std::complex<double>)));
            CHECK_CUDA_ERROR(
                cudaMalloc(&V_buff, m * n * sizeof(std::complex<double>)));
            CHECK_CUDA_ERROR(
                cudaMalloc(&W_buff, m * n * sizeof(std::complex<double>)));
            CHECK_CUDA_ERROR(
                cudaMalloc(&V2_buff, m * n * sizeof(std::complex<double>)));
            CHECK_CUDA_ERROR(
                cudaMalloc(&W2_buff, m * n * sizeof(std::complex<double>)));
...
TYPED_TEST(HEMMGPUDistTest, HEMMDistCorrectnessCUDAAwareGPU)
{
    ...
    chase::linalg::internal::cuda_mpi::MatrixMultiplyMultiVectors(
        this->get_cublas_handle(), &alpha, H_, V_, &beta, W_, offset, subSize);
    W_.D2H();
    ...
    chase::linalg::internal::cuda_mpi::MatrixMultiplyMultiVectors(
        this->get_cublas_handle(), &alpha, H_, W_, &beta, V_, offset, subSize);
...
TYPED_TEST(HEMMGPUDistTest, HEMMRedistribeAsyncDistCorrectness)
{
    ...
    auto V2_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::column,
        chase::platform::GPU>(this->m, this->n, this->m,
                              reinterpret_cast<T*>(this->get_V2_buff()),
                              this->get_mpi_grid());
    auto W2_ = chase::distMultiVector::DistMultiVector1D<
        T, chase::distMultiVector::CommunicatorType::row, chase::platform::GPU>(
        this->m, this->n, this->m, reinterpret_cast<T*>(this->get_W2_buff()),
        this->get_mpi_grid());
    ...
    chase::linalg::internal::cuda_mpi::
        MatrixMultiplyMultiVectorsAndRedistribute(
            this->get_cublas_handle(), H_, V_, W_, V2_, W2_, offset, subSize);
    ...
    for (auto i = 0; i < W2_.l_cols(); i++)
    {
        for (auto j = 0; j < W2_.l_rows(); j++)
        {
            ...
            W2_.cpu_data()[i * W2_.cpu_ld() + j] =
                W_.cpu_data()[i * W2_.cpu_ld() + j];
        }
    }
    W2_.H2D();
    chase::linalg::internal::cuda_mpi::
        MatrixMultiplyMultiVectorsAndRedistribute(
            this->get_cublas_handle(), H_, W_, V_, W2_, V2_, offset, subSize);
