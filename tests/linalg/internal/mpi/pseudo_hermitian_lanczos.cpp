...
TYPED_TEST(PseudoHermitianLanczosCPUDistTest, tinyPseudoHermitianLanczos)
{
    ...
    auto V_ = chase::distMultiVector::DistMultiVector1D<
        ...
        this->N_tiny, this->M_tiny, mpi_grid);
    ...
    chase::linalg::internal::cpu_mpi::lanczos_dispatch(
        this->M_tiny, this->numvec_tiny, H_, V_, &upperb,
        this->ritzv_tiny.data(), this->Tau_tiny.data(),
        this->ritzV_tiny.data());

    chase::Base<T> diff_min =
        std::norm(this->ritzv_tiny[0] - std::real(exact_eigsl_H.data()[0]));
    chase::Base<T> diff_max =
        std::norm(this->ritzv_tiny[this->M_tiny - 1] -
...
TYPED_TEST(PseudoHermitianLanczosCPUDistTest,
           tinyPseudoHermitianSimplifiedLanczos)
{
    using T = TypeParam;
    ...
    chase::linalg::internal::cpu_mpi::lanczos_dispatch(this->M_tiny, H_, V_,
                                                       &upperb);

    chase::Base<T> actual_upperb =
        (chase::Base<T>)std::real(exact_eigsl_H.data()[this->N_tiny - 1]);
    ...
    EXPECT_LT(upperb, 5 * (chase::Base<T>)std::real(
                              exact_eigsl_H.data()[this->N_tiny - 1]));
}
...
TYPED_TEST(PseudoHermitianLanczosCPUDistTest, PseudoHermitianLanczos)
{
    ...
    chase::Base<T> diff_min =
        std::norm(this->ritzv[0] - std::real(exact_eigsl_H.data()[0]));
    chase::Base<T> diff_max =
...
TYPED_TEST(PseudoHermitianLanczosCPUDistTest, PseudoHermitianSimplifiedLanczos)
{
    ...
    chase::linalg::internal::cpu_mpi::lanczos_dispatch(this->M, H_, V_,
                                                       &upperb);

    chase::Base<T> actual_upperb =
        (chase::Base<T>)std::real(exact_eigsl_H.data()[this->N - 1]);
    EXPECT_TRUE(upperb >= actual_upperb ||
                std::abs(upperb - actual_upperb) / std::abs(actual_upperb) <=
                    1e-2)
        << "Value " << upperb << " is below the acceptable threshold of "
    ...
    EXPECT_LT(upperb,
              5 * (chase::Base<T>)std::real(exact_eigsl_H.data()[this->N - 1]));
}
