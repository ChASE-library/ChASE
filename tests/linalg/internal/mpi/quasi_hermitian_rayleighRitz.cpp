...
TYPED_TEST(QuasiHermitianRRCPUDistTest, QuasiHermitianRRTinyCorrectness) {
    ...
    auto V1_ = chase::distMultiVector::DistMultiVector1D<T, chase::distMultiVector::CommunicatorType::column>(this->N_tiny, this->N_tiny, mpi_grid);
    auto V2_ = chase::distMultiVector::DistMultiVector1D<T, chase::distMultiVector::CommunicatorType::column>(this->N_tiny, this->N_tiny, mpi_grid);
    
    auto W1_ = chase::distMultiVector::DistMultiVector1D<T, chase::distMultiVector::CommunicatorType::row>(this->N_tiny, this->N_tiny, mpi_grid);
    auto W2_ = chase::distMultiVector::DistMultiVector1D<T, chase::distMultiVector::CommunicatorType::row>(this->N_tiny, this->N_tiny, mpi_grid);
    ...
    {
        EXPECT_NEAR(this->ritzv_tiny.data()[i], chase::Base<T>(std::real(exact_eigsl_H.data()[i])), 100 * MachineEpsilon<T>::value());
    }
}
...
TYPED_TEST(QuasiHermitianRRCPUDistTest, QuasiHermitianRRCorrectness) {
    ...
    auto V1_ = chase::distMultiVector::DistMultiVector1D<T, chase::distMultiVector::CommunicatorType::column>(this->N, this->N, mpi_grid);
    auto V2_ = chase::distMultiVector::DistMultiVector1D<T, chase::distMultiVector::CommunicatorType::column>(this->N, this->N, mpi_grid);
    
    auto W1_ = chase::distMultiVector::DistMultiVector1D<T, chase::distMultiVector::CommunicatorType::row>(this->N, this->N, mpi_grid);
    ...
    for(auto i = offset; i < offset + subSize; i++)
    {
        EXPECT_NEAR(this->ritzv.data()[i], chase::Base<T>(std::real(exact_eigsl_H.data()[i])), tolerance);
    }
}
