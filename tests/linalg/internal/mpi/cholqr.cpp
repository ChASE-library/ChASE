...
TYPED_TEST(CHOLQRCPUDistTest, cholQR1)
{
    ...
    read_vectors(V.data(), GetQRFileName<T>() + "cond_10.bin", xoff, xlen,
                 this->N, this->n, 0);
    int info = chase::linalg::internal::cpu_mpi::cholQR1<T>(
        xlen, this->n, V.data(), xlen, MPI_COMM_WORLD);
    ASSERT_EQ(info, 0);
...
TYPED_TEST(CHOLQRCPUDistTest, cholQR2)
{
    ...
    read_vectors(V.data(), GetQRFileName<T>() + "cond_1e4.bin", xoff, xlen,
                 this->N, this->n, 0);
    int info = chase::linalg::internal::cpu_mpi::cholQR2<T>(
        xlen, this->n, V.data(), xlen, MPI_COMM_WORLD);
    ASSERT_EQ(info, 0);
    auto orth = orthogonality<T>(xlen, this->n, V.data(), xlen, MPI_COMM_WORLD);
    ASSERT_NEAR(orth, machineEpsilon, machineEpsilon * 15);
...
TYPED_TEST(CHOLQRCPUDistTest, scholQR)
{
    ...
    read_vectors(V.data(), GetQRFileName<T>() + "cond_ill.bin", xoff, xlen,
                 this->N, this->n, 0);
    int info = chase::linalg::internal::cpu_mpi::shiftedcholQR2<T>(
        this->N, xlen, this->n, V.data(), xlen, MPI_COMM_WORLD);
    ASSERT_EQ(info, 0);
    auto orth = orthogonality<T>(xlen, this->n, V.data(), xlen, MPI_COMM_WORLD);
...
TYPED_TEST(CHOLQRCPUDistTest, scalapackHHQR)
{
    ...
    auto machineEpsilon = MachineEpsilon<T>::value();
    read_vectors(V_.l_data(), GetQRFileName<T>() + "cond_ill.bin", xoff, xlen,
                 this->N, this->n, 0);
    chase::linalg::internal::cpu_mpi::houseHoulderQR(V_);
    auto orth =
        orthogonality<T>(xlen, this->n, V_.l_data(), xlen, MPI_COMM_WORLD);
    ASSERT_NEAR(orth, machineEpsilon, machineEpsilon * 10);
}
#endif