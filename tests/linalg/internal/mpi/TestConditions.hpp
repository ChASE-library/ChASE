...
template<typename T>
chase::Base<T> orthogonality(std::size_t m, std::size_t n, T *V, std::size_t ldv, MPI_Comm comm)
{
    T one = T(1.0);
    T negone = T(-1.0);
    T zero = T(0.0);
    ...
    MPI_Allreduce(MPI_IN_PLACE, A.data(), n * n, chase::mpi::getMPI_Type<T>(), MPI_SUM, comm);
