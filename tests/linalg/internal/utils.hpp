...
{
    return GetBSE_Path<T>() + "/eigs_" + TypeName<T>::Get() +
           "tiny_random_BSE.bin";
}
...
{
    return GetBSE_Path<T>() + "/SH_eigs_" + TypeName<T>::Get() +
           "random_BSE.bin";
}
...
template <typename T>
void read_vectors(T* Vec, std::string path_in, std::size_t xoff,
                  std::size_t xlen, std::size_t M, std::size_t nevex, int rank)
{
