...
TYPED_TEST(rayleighRitzCPUTest, eigenpairs)
{
    ...
    chase::matrix::Matrix<T>* H2 =
        new chase::matrix::Matrix<T>(this->N, this->N);
    ...
    chase::linalg::internal::cpu::rayleighRitz(H2, this->n, this->H.data(),
                                               this->N, this->W.data(), this->N,
                                               this->ritzv.data());
    ...
    T beta;
    chase::linalg::blaspp::t_gemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                  this->N, this->n, this->N, &One, H2->data(),
                                  this->N, this->W.data(), this->N, &Zero,
                                  this->Q.data(), this->N);

    for (std::size_t i = 0; i < this->n; ++i)
    {
        beta = -T(this->ritzv[i]);
        chase::linalg::blaspp::t_axpy(this->N, &beta,
                                      this->W.data() + this->N * i, 1,
                                      this->Q.data() + this->N * i, 1);

        this->resids[i] = chase::linalg::blaspp::t_nrm2(
            this->N, this->Q.data() + this->N * i, 1);
