...
TYPED_TEST(CholQRCPUTest, cholQR1) {
    ...
    int info = chase::linalg::internal::cpu::cholQR1<T>(this->m, this->n, this->V.data(), this->m);
    ASSERT_EQ(info, 0);
...
TYPED_TEST(CholQRCPUTest, cholQR2) {
    ...
    auto machineEpsilon = MachineEpsilon<T>::value();
    read_vectors(this->V.data(), GetQRFileName<T>() + "cond_1e4.bin", 0, this->m, this->m, this->n, 0);
    int info = chase::linalg::internal::cpu::cholQR2<T>(this->m, this->n, this->V.data(), this->m);
    ASSERT_EQ(info, 0);
    auto orth = orthogonality<T>(this->m, this->n, this->V.data(), this->m);
    ASSERT_NEAR(orth, machineEpsilon, machineEpsilon * 15);
}
...
TYPED_TEST(CholQRCPUTest, scholQR) {
    ...
    auto machineEpsilon = MachineEpsilon<T>::value();
    read_vectors(this->V.data(), GetQRFileName<T>() + "cond_ill.bin", 0, this->m, this->m, this->n, 0);
    int info = chase::linalg::internal::cpu::shiftedcholQR2<T>(this->m, this->n, this->V.data(), this->m);
    ASSERT_EQ(info, 0);
    auto orth = orthogonality<T>(this->m, this->n, this->V.data(), this->m);
