...
TYPED_TEST(LaczosCPUTest, tinyQuasiHermitianSimplefiedLanczos) {
    ...
    chase::linalg::internal::cpu::lanczos(nHmat, Hmat, V_lanczos->data(), nHmat, &upperb);
    
    delete Hmat;
    delete V_lanczos;
    
    EXPECT_GT(upperb,  (chase::Base<T>)std::real(eigsl_H->data()[nHmat-1]));
    EXPECT_LT(upperb,5*(chase::Base<T>)std::real(eigsl_H->data()[nHmat-1]));
...
TYPED_TEST(LaczosCPUTest, tinyQuasiHermitianLanczos) {
    ...
    chase::linalg::internal::cpu::lanczos(nHmat, 1, Hmat, V_lanczos->data(), nHmat,
...
TYPED_TEST(LaczosCPUTest, tinyFlipedQuasiHermitianSimplefiedLanczos) {
    using T = std::complex<double>;
    ...
    EXPECT_GT(upperb,  (chase::Base<T>)std::real(eigsl_SH->data()[nHmat-1]));
    EXPECT_LT(upperb,5*(chase::Base<T>)std::real(eigsl_SH->data()[nHmat-1]));
...
TYPED_TEST(LaczosCPUTest, tinyFlipedQuasiHermitianLanczos) {
    ...
    chase::linalg::internal::cpu::lanczos(nHmat,1,Hmat, V_lanczos->data(),V_lanczos->ld(),&upperb,Theta,Tau,ritzV);
