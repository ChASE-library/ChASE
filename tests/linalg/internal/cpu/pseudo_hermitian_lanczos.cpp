...
template <typename T>
class PseudoHermitianLanczosCPUTest : public ::testing::Test
{
protected:
    void SetUp() override
    {
        ...
        H = new chase::matrix::PseudoHermitianMatrix<T, chase::platform::CPU>(
            N, N);
        exact_eigsl_H =
            new chase::matrix::Matrix<T, chase::platform::CPU>(N, 1);
        ...
        H_tiny =
            new chase::matrix::PseudoHermitianMatrix<T, chase::platform::CPU>(
                N_tiny, N_tiny);
        exact_eigsl_H_tiny =
            new chase::matrix::Matrix<T, chase::platform::CPU>(N_tiny, 1);
...
TYPED_TEST_SUITE(PseudoHermitianLanczosCPUTest, TestTypes);

TYPED_TEST(PseudoHermitianLanczosCPUTest, tinyPseudoHermitianSimplefiedLanczos)
{
    using T = TypeParam;
    ...
    chase::linalg::internal::cpu::lanczos<T>(
        this->M_tiny, this->H_tiny, this->V_tiny.data(), this->N_tiny, &upperb);

    chase::Base<T> actual_upperb = (chase::Base<T>)std::real(
        this->exact_eigsl_H_tiny->data()[this->N_tiny - 1]);
    EXPECT_TRUE(upperb >= actual_upperb ||
                std::abs(upperb - actual_upperb) / std::abs(actual_upperb) <=
                    1e-2)
        << "Value " << upperb << " is below the acceptable threshold of "
        << actual_upperb << "\n";

    EXPECT_LT(upperb,
              5 * (chase::Base<T>)std::real(
                      this->exact_eigsl_H_tiny->data()[this->N_tiny - 1]));
}

TYPED_TEST(PseudoHermitianLanczosCPUTest, tinyPseudoHermitianLanczos)
{
    using T = TypeParam;
    ...
    chase::linalg::internal::cpu::lanczos<T>(
        this->M_tiny, this->numvec_tiny, this->H_tiny, this->V_tiny.data(),
        this->N_tiny, &upperb, this->ritzv_tiny.data(), this->Tau_tiny.data(),
        this->ritzV_tiny.data());

    chase::Base<T> diff_min = std::norm(
        this->ritzv_tiny[0] - std::real(this->exact_eigsl_H_tiny->data()[0]));
    chase::Base<T> diff_max = std::norm(
...
TYPED_TEST(PseudoHermitianLanczosCPUTest, PseudoHermitianSimplefiedLanczos)
{
    using T = TypeParam;
    ...
    chase::linalg::internal::cpu::lanczos<T>(this->M, this->H, this->V.data(),
                                             this->N, &upperb);

    chase::Base<T> actual_upperb =
        (chase::Base<T>)std::real(this->exact_eigsl_H->data()[this->N - 1]);
    EXPECT_TRUE(upperb >= actual_upperb ||
                std::abs(upperb - actual_upperb) / std::abs(actual_upperb) <=
                    1e-2)
        << "Value " << upperb << " is below the acceptable threshold of "
    ...
    EXPECT_LT(upperb, 5 * (chase::Base<T>)std::real(
                              this->exact_eigsl_H->data()[this->N - 1]));
}

TYPED_TEST(PseudoHermitianLanczosCPUTest, PseudoHermitianLanczos)
{
    using T = TypeParam;
    ...
    chase::Base<T> diff_min =
        std::norm(this->ritzv[0] - std::real(this->exact_eigsl_H->data()[0]));
    chase::Base<T> diff_max =
        std::norm(this->ritzv[this->M - 1] -
                  std::real(this->exact_eigsl_H->data()[this->N - 1]));
