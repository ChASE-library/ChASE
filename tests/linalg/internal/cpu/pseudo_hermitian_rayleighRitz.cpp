...
TYPED_TEST(PseudoHermitianRayleighRitzCPUTest, PseudoHermitianRayleighRitz)
{
    ...
    chase::linalg::internal::cpu::rayleighRitz(
        this->H, this->nev, this->Q.data(), this->N, this->V.data(), this->N,
        this->ritzv.data(), this->Workspace.data());

    std::sort(this->ritzv.begin(), this->ritzv.end());

    for (auto i = 0; i < this->nev; i++)
    {
        EXPECT_LT(this->ritzv.data()[i],
                  std::real(this->exact_eigsl_H->data()[i]) +
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
        EXPECT_GT(this->ritzv.data()[i],
                  std::real(this->exact_eigsl_H->data()[i]) -
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
    }
...
TYPED_TEST(PseudoHermitianRayleighRitzCPUTest, tinyPseudoHermitianRayleighRitz)
{
    ...
    chase::linalg::internal::cpu::rayleighRitz(
        this->H_tiny, this->nev_tiny, this->Q_tiny.data(), this->N_tiny,
        this->V_tiny.data(), this->N_tiny, this->ritzv_tiny.data(),
        this->Workspace_tiny.data());

    std::sort(this->ritzv_tiny.begin(), this->ritzv_tiny.end());

    for (auto i = 0; i < this->nev_tiny; i++)
    {
        EXPECT_LT(this->ritzv_tiny.data()[i],
                  std::real(this->exact_eigsl_H_tiny->data()[i]) +
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
        EXPECT_GT(this->ritzv_tiny.data()[i],
                  std::real(this->exact_eigsl_H_tiny->data()[i]) -
                      GetErrorTolerance<
                          T>()); // MachineEpsilon<chase::Base<T>>::value());
    }
