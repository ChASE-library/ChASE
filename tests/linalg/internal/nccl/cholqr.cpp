...
TYPED_TEST(CHOLQRNCCLDistTest, cholQR1GPU) {
    ...
    int info = chase::linalg::internal::cuda_nccl::cholQR1<T>(this->get_cublas_handle(), this->get_cusolver_handle(), xlen, this->n, reinterpret_cast<T*>(this->get_device_vector()), xlen, this->get_mpi_grid()->get_nccl_comm());
    ASSERT_EQ(info, 0);
...
TYPED_TEST(CHOLQRNCCLDistTest, cholQR2GPU) {
    ...
    read_vectors(V.data(), GetQRFileName<T>() + "cond_1e4.bin", xoff, xlen, this->N, this->n, 0);
    CHECK_CUDA_ERROR(cudaMemcpy((reinterpret_cast<T*>(this->get_device_vector())), V.data(), sizeof(T) * xlen * this->n, cudaMemcpyHostToDevice));
    int info = chase::linalg::internal::cuda_nccl::cholQR2<T>(this->get_cublas_handle(), this->get_cusolver_handle(), xlen, this->n, reinterpret_cast<T*>(this->get_device_vector()), xlen, this->get_mpi_grid()->get_nccl_comm());
    ASSERT_EQ(info, 0);
    CHECK_CUDA_ERROR(cudaMemcpy(V.data(), reinterpret_cast<T*>(this->get_device_vector()), sizeof(T) * xlen * this->n, cudaMemcpyDeviceToHost));
    auto orth = orthogonality<T>(xlen, this->n, V.data(), xlen, MPI_COMM_WORLD);
    ASSERT_NEAR(orth, machineEpsilon, machineEpsilon * 15);
...
TYPED_TEST(CHOLQRNCCLDistTest, scholQRGPU) {
    ...
    read_vectors(V.data(), GetQRFileName<T>() + "cond_ill.bin", xoff, xlen, this->N, this->n, 0);
    CHECK_CUDA_ERROR(cudaMemcpy((reinterpret_cast<T*>(this->get_device_vector())), V.data(), sizeof(T) * xlen * this->n, cudaMemcpyHostToDevice));
    int info = chase::linalg::internal::cuda_nccl::shiftedcholQR2<T>(this->get_cublas_handle(), this->get_cusolver_handle(), this->N, xlen, this->n, reinterpret_cast<T*>(this->get_device_vector()), xlen, this->get_mpi_grid()->get_nccl_comm());
    ASSERT_EQ(info, 0);
    CHECK_CUDA_ERROR(cudaMemcpy(V.data(), reinterpret_cast<T*>(this->get_device_vector()), sizeof(T) * xlen * this->n, cudaMemcpyDeviceToHost));
    auto orth = orthogonality<T>(xlen, this->n, V.data(), xlen, MPI_COMM_WORLD);
    ASSERT_NEAR(orth, machineEpsilon, machineEpsilon * 15);
