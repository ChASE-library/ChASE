...
TYPED_TEST(flipSignGPUNCCLDistTest, FlipSignCorrectnessGPU)
{
    ...
    chase::linalg::internal::cuda_nccl::flipLowerHalfMatrixSign(H, offset,
                                                                subSize);
...
TYPED_TEST(flipSignGPUNCCLDistTest, FlipSignBlockCyclicCorrectnessGPU)
{
    ...
    auto H = chase::distMatrix::PseudoHermitianBlockCyclicMatrix<
        T, chase::platform::GPU>(N, N, mb, mb, mpi_grid);
    H.allocate_cpu_data();
                        ...
                        EXPECT_EQ(
                            H.cpu_data()[(j +
                                          H.n_contiguous_local_offs()[idx_j]) *
                                             H.cpu_ld() +
                                         i +
                                         H.m_contiguous_local_offs()[idx_i]],
                            T(1.0));
                    }
                    else
                    {
...
TYPED_TEST(flipSignGPUNCCLDistTest,
           FlipSignColMultiVectorsBlockBlockCorrectnessGPU)
{
    using T = TypeParam; // Get the current type
    ...
    auto V = chase::distMultiVector::DistMultiVector1D<
        ...
        chase::platform::GPU>(N, n, mpi_grid);
    V.allocate_cpu_data();
    ...
    chase::linalg::internal::cuda_nccl::flipLowerHalfMatrixSign(V, offset,
                                                                subSize);
...
TYPED_TEST(flipSignGPUNCCLDistTest,
           FlipSignRowMultiVectorsBlockBlockCorrectnessGPU)
{
    using T = TypeParam; // Get the current type
...
TYPED_TEST(flipSignGPUNCCLDistTest,
           FlipSignColMultiVectorsBlockCyclicCorrectnessGPU)
{
    using T = TypeParam; // Get the current type
    ...
    auto V = chase::distMultiVector::DistMultiVectorBlockCyclic1D<
        T, chase::distMultiVector::CommunicatorType::column,
        chase::platform::GPU>(N, n, mb, mpi_grid);
    V.allocate_cpu_data();
        ...
        for (auto i = 0; i < V.m_contiguous_lens()[idx_i]; i++)
        {
            for (auto j = 0; j < V.l_cols(); j++)
            {
                if (i + V.m_contiguous_global_offs()[idx_i] < (N / 2) ||
                    j < offset)
                {
                    EXPECT_EQ(V.cpu_data()[j * V.cpu_ld() + i +
...
TYPED_TEST(flipSignGPUNCCLDistTest,
           FlipSignRowMultiVectorsBlockCyclicCorrectnessGPU)
{
    using T = TypeParam; // Get the current type
