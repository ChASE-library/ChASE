...
template <typename T>
class PseudoRayleighRitzGPUNCCLDistTest : public ::testing::Test
{
protected:
    void SetUp() override
    {
...
TYPED_TEST_SUITE(PseudoRayleighRitzGPUNCCLDistTest, TestTypes);

TYPED_TEST(PseudoRayleighRitzGPUNCCLDistTest,
           TinyPseudoHermitianRRDistGPUCorrectness)
{
    ...
    {
        tolerance =
            1.0e-2; // Very small large tolerance because full basis experiment
    }
    else
    {
        tolerance = 1.0e-9;
    ...
    auto H_ = chase::distMatrix::PseudoHermitianBlockBlockMatrix<
        T, chase::platform::GPU>(this->N_tiny, this->N_tiny, mpi_grid);
    H_.allocate_cpu_data();
    H_.readFromBinaryFile(GetBSE_TinyMatrix<T>());
    H_.H2D();

    chase::distMatrix::RedundantMatrix<chase::Base<T>, chase::platform::GPU>
        ritzv_tiny = chase::distMatrix::RedundantMatrix<chase::Base<T>,
                                                        chase::platform::GPU>(
            this->N_tiny, 1, mpi_grid);
    ritzv_tiny.allocate_cpu_data();

    chase::matrix::Matrix<T, chase::platform::CPU> exact_eigsl_H =
        chase::matrix::Matrix<T, chase::platform::CPU>(this->N_tiny, 1);
    ...
    auto V1_ = chase::distMultiVector::DistMultiVector1D<
        ...
        chase::platform::GPU>(this->N_tiny, this->N_tiny, mpi_grid);
    ...
    chase::linalg::internal::cuda_nccl::pseudo_hermitian_rayleighRitz(
        this->get_cublas_handle(), this->get_cusolver_handle(),
        this->get_cusolver_params(), H_, V1_, V2_, W1_, W2_, ritzv_tiny, offset,
        subSize, devInfo);

    for (auto i = offset; i < offset + subSize; i++)
    {
        EXPECT_NEAR(ritzv_tiny.cpu_data()[i],
...
TYPED_TEST(PseudoRayleighRitzGPUNCCLDistTest,
           PseudoHermitianRRDistGPUCorrectness)
{
    using T = TypeParam;            // Get the current type
    ASSERT_EQ(this->world_size, 4); // Ensure we're running with 4 processes
    std::shared_ptr<chase::grid::MpiGrid2D<chase::grid::GridMajor::ColMajor>>
    ...
    auto H_ = chase::distMatrix::PseudoHermitianBlockBlockMatrix<
        T, chase::platform::GPU>(this->N, this->N, mpi_grid);
    H_.allocate_cpu_data();
    H_.readFromBinaryFile(GetBSE_Matrix<T>());
    H_.H2D();

    chase::distMatrix::RedundantMatrix<chase::Base<T>, chase::platform::GPU>
        ritzv = chase::distMatrix::RedundantMatrix<chase::Base<T>,
                                                   chase::platform::GPU>(
            this->N, 1, mpi_grid);
    ritzv.allocate_cpu_data();

    chase::matrix::Matrix<T, chase::platform::CPU> exact_eigsl_H =
        chase::matrix::Matrix<T, chase::platform::CPU>(this->N, 1);
    ...
    chase::linalg::internal::cuda_nccl::pseudo_hermitian_rayleighRitz(
        this->get_cublas_handle(), this->get_cusolver_handle(),
        this->get_cusolver_params(), H_, V1_, V2_, W1_, W2_, ritzv, offset,
        subSize, devInfo);

    for (auto i = offset; i < offset + subSize; i++)
    {
        EXPECT_NEAR(ritzv.cpu_data()[i],
                    chase::Base<T>(std::real(exact_eigsl_H.data()[i])),
                    tolerance);
    }
}
