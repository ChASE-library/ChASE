 ...
 * This class implements the CPU-based parallel version of the Chase algorithm
 * using MPI. It inherits from `ChaseBase` and provides methods for solving
 * generalized eigenvalue problems with MPI-based parallelism. The class
 * operates on matrix and multi-vector data types, leveraging MPI for
 * communication across different processes in a distributed computing
 * environment.
 *
 * @section constructor_sec Constructors and Destructor
 * The constructor and destructor for the `pChASECPU` class manage the
 * initialization of the matrix data, multi-vectors, and MPI communication. The
 * constructor also ensures that the matrix is square and that the matrix and
 * eigenvectors are mapped to the same MPI grid.
 *
 * @section members_sec Private Members
 * Private members include matrix data, multi-vectors, configuration settings,
 * and MPI-specific information such as rank, size, coordinates, and dimensions.
 * These members are initialized during construction.
 */

/**
 * @brief CPU-based parallel Chase algorithm with MPI.
 *
 * This class solves generalized eigenvalue problems using a parallel
 * implementation of the Chase algorithm. It leverages MPI for parallel
 * processing and works with matrix and multi-vector data types.
 *
...
template <typename MatrixType, typename InputMultiVectorType,
          typename BackendType = chase::grid::backend::MPI>
...
public:
     ...
     *
     * Initializes the Chase algorithm with the given matrix, eigenvector, and
     * Ritz values. The matrix and eigenvectors must be mapped to the same MPI
     * grid. The constructor also verifies that the matrix is square and sets up
     * the necessary data structures for the algorithm.
     *
     ...
     */
    pChASECPU(std::size_t nev, std::size_t nex, MatrixType* H,
          ...
          config_(H->g_rows(), nev, nex), N_(H->g_rows())
    {
        ...
        // V2_ = new InputMultiVectorType(Hmat_->g_rows(), nevex_,
        // Hmat_->getMpiGrid_shared_ptr());
     ...
     *
     * The copy constructor is deleted to prevent copying of this class, as it
     * manages unique resources such as MPI communication.
     */
    pChASECPU(const pChASECPU&) = delete;
    /**
     * @brief Destructor for the pChASECPU class.
     *
     * Cleans up any resources allocated by the constructor, including matrix
     * data and MPI-specific information.
     */
    ~pChASECPU() {}
    ...
    bool checkPseudoHermicityEasy() override { return is_pseudoHerm_; }
    ...
    void Lanczos(std::size_t m, chase::Base<T>* upperb) override
    {
        lanczosIter_ = m;
        numLanczos_ = 1;
        chase::linalg::internal::cpu_mpi::lanczos_dispatch(m, *Hmat_, *V1_,
                                                           upperb);
    }
                 ...
                 chase::Base<T>* ritzV) override
    {
        lanczosIter_ = M;
        numLanczos_ = numvec;
        chase::linalg::internal::cpu_mpi::lanczos_dispatch(
            M, numvec, *Hmat_, *V1_, upperb, ritzv, Tau, ritzV);
    }
    ...
    void LanczosDos(std::size_t idx, std::size_t m, T* ritzVc) override
    {
        ...
        chase::linalg::blaspp::t_gemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                                      V1_->l_rows(), idx, m, &One,
                                      V1_->l_data(), V1_->l_ld(), ritzVc, m,
                                      &Zero, V2_->l_data(), V2_->l_ld());

        chase::linalg::lapackpp::t_lacpy('A', V2_->l_rows(), m, V2_->l_data(),
                                         V2_->l_ld(), V1_->l_data(),
                                         V1_->l_ld());
    }
    ...
    void Shift(T c, bool isunshift = false) override
    {
        ...
        {
            auto min = *std::min_element(resid_->l_data() + locked_,
                                         resid_->l_data() + nev_);
    ...
    void HEMM(std::size_t block, T alpha, T beta, std::size_t offset) override
    {
                      ...
                      std::is_same<T, std::complex<double>>::value)
        {
            ...
            if (min > 1e-3)
            {
                ...
                // static_assert(std::is_same_v<std::remove_pointer_t<decltype(V1_sp)>,
                // chase::distMultiVector::DistMultiVectorBlockCyclic1D<std::complex<float>,
                // chase::distMultiVector::CommunicatorType::column,
                // chase::platform::CPU>>, "Type mismatch in singlePrecisionT");
    ...
    void QR(std::size_t fixednev, chase::Base<T> cond) override
    {
        ...
        {
            auto V_tmp = V1_->template clone2<InputMultiVectorType>(
                V1_->g_rows(), V1_->g_cols() - locked_);
            std::memcpy(V_tmp->l_data(), V1_->l_data() + locked_ * V1_->l_ld(),
        ...
        else
        {
            ...
            if (cond > cond_threshold_upper)
            {
                              ...
                              std::is_same<T, float>::value)
                {
                    ...
                    info = chase::linalg::internal::cpu_mpi::shiftedcholQR2(
                        V1_->g_rows(), V1_d->l_rows(), V1_d->l_cols(),
                        V1_d->l_data(), V1_d->l_ld(),
                        V1_->getMpiGrid()->get_col_comm(), A_d->l_data());
                    V1_->copyback();
                }
                else
                {
            ...
            else if (cond < cond_threshold_lower)
            {
                              ...
                              std::is_same<T, float>::value)
                {
                    ...
                    auto V1_d = V1_->getDoublePrecisionMatrix();
                    info = chase::linalg::internal::cpu_mpi::cholQR1(
                        V1_d->l_rows(), V1_d->l_cols(), V1_d->l_data(),
                        V1_d->l_ld(), V1_->getMpiGrid()->get_col_comm());
                    V1_->copyback();
                }
                else
                {
#endif
                    info = chase::linalg::internal::cpu_mpi::cholQR1(
                        V1_->l_rows(), V1_->l_cols(), V1_->l_data(),
                        V1_->l_ld(), V1_->getMpiGrid()->get_col_comm());
#ifdef QR_DOUBLE_PRECISION
                }
#endif
            }
            else
            {
                              ...
                              std::is_same<T, float>::value)
                {
                    ...
                    auto V1_d = V1_->getDoublePrecisionMatrix();
                    auto A_d = A_->getDoublePrecisionMatrix();
                    info = chase::linalg::internal::cpu_mpi::cholQR2(
                        V1_d->l_rows(), V1_d->l_cols(), V1_d->l_data(),
                        V1_d->l_ld(), V1_->getMpiGrid()->get_col_comm(),
                        A_d->l_data());
                    V1_->copyback();
...
private:
     ...
     *
     * This enum specifies the two possible operations in the Chase algorithm:
     * `cAb` and `bAc`.
     */
    enum NextOp
    {
     ...
     *
     * This member stores the next operation to be executed based on the
     * algorithm's current step. It can be set to either `cAb` or `bAc`,
     * depending on the context of the computation.
     */
    NextOp next_;
     ...
     *
     * This boolean value is used to track whether the matrix being processed is
     * symmetric. It influences certain algorithmic steps to optimize
     ...
     *
     * This boolean value is used to track whether the matrix being processed is
     * pseudo-hermitian. It influences certain algorithmic steps to optimize
     * performance or correctness.
     */
    bool is_pseudoHerm_;
     ...
     *
     * This member holds the number of eigenvalues (nev_) that the algorithm
     * will compute.
     */
    std::size_t nev_;
     ...
     *
     * This member holds the number of extra eigenvalues (nex_) to compute in
     * addition to nev_.
     */
    std::size_t nex_;
     ...
     *
     * This member holds the total number of eigenvalues to compute, which is
     * the sum of `nev_` and `nex_`.
     */
    std::size_t nevex_;
     ...
     *
     * This value indicates whether certain parameters are locked, preventing
     * changes during the algorithm's execution.
     */
    std::size_t locked_;
     ...
     *
     * This member holds the number of rows in the matrix H (also referred to as
     * `N_`).
     */
    std::size_t N_;
     ...
     *
     * This integer stores the total number of processes involved in the
     ...
     *
     * This integer stores the rank of the current process within the MPI
     * communicator.
     */
    int my_rank_;
     ...
     *
     * This pointer stores the coordinates of the current process in the MPI
     * grid.
     */
    int* coords_;
     ...
     *
     * This pointer stores the dimensions of the MPI grid, used for distributing
     ...
     *
     * This member points to the matrix `H` that is being solved in the
     * algorithm.
     */
    MatrixType* Hmat_;
     ...
     *
     * This unique pointer holds a copy of the input multi-vector `V1_`, used
     ...
     *
     * This unique pointer holds a result multi-vector that is used for storing
     ...
     *
     * This unique pointer holds another result multi-vector for storing
     * additional intermediate computation results.
     */
     ...
     *
     * This unique pointer holds the Ritz values, which are used in the Chase
     * algorithm's eigenvalue computation.
     */
     ...
     *
     * This unique pointer stores the residuals of the computed eigenvalues
     * during the Chase algorithm.
     */
     ...
     *
     * This unique pointer holds an auxiliary matrix used in the intermediate
     * steps of the Chase algorithm.
     */
     ...
     *
     * This member holds the configuration settings for the Chase algorithm,
     * such as parameters for matrix dimensions and other algorithmic settings.
     */
