...
template < class T >
std::size_t
ChASE_Algorithm<T>::calc_degrees(
                              ChASE< T > *single,
                              std::size_t N,
  ...
  while( unconverged > nex && iteration < config.getMaxIter() )
    {
      ...
      std::cout
        << std::scientific
        << "iteration: " << iteration
        << "\t" << std::setprecision(6) << lambda << "\t" << std::setprecision(6) << lowerb << "\t" << std::setprecision(6) << upperb
        << "\t" << unconverged
        << std::endl;
#endif
