...
template <class T>
void
swap_kj(std::size_t k, std::size_t j, T* array)
{
  T tmp = array[k];
  array[k] = array[j];
  array[j] = tmp;
...
template <class T>
std::size_t
ChASE_Algorithm<T>::calc_degrees(ChASE<T>* single, std::size_t N,
...
template <class T>
std::size_t
ChASE_Algorithm<T>::locking(ChASE<T>* single, std::size_t N,
...
template <class T>
std::size_t
ChASE_Algorithm<T>::filter(ChASE<T>* single, std::size_t n,
...
template <class T>
std::size_t
ChASE_Algorithm<T>::lanczos(ChASE<T>* single, int N, int numvec, int m,
                            ...
                            Base<T>* ritzv_)
{
  ...
  Base<T> upperb_ = std::numeric_limits<Base<T>>::min();
  ...
  for (std::size_t i = 0; i < numvec; ++i) {
    ...
    /*     V_[k] = T( d(gen), d(gen) ); */
    /*   } */
    single->lanczos(m, &upperb_, Theta + m * i, Tau + m * i, ritzV);
  ...
  const double search =
    static_cast<double>(nevex + single->getNex() / 2) / static_cast<double>(N);
...
template <class T>
ChASE_PerfData
ChASE_Algorithm<T>::solve(ChASE<T>* single, std::size_t N, Base<T>* ritzv_,
...
//--------------------------------- FILTER ---------------------------------
    ...
    std::size_t new_converged =
      locking(single, N, unconverged, tol, ritzv, resid, degrees, locked);
