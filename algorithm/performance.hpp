...
public:
    ...
    void Reset()
    {
        ...
        {
            start_points[i].erase(start_points[i].begin(),
                                  start_points[i].end());
     ...
     */
    std::size_t get_flops(std::size_t N = 0, std::size_t lanczosIter = 0,
                          std::size_t numLanczos = 0)
    {
        // Lanczos //
        // GEMM operations
        ...
        // stemr solve with eigenvectors O(n2) -
        // https://www.cs.utexas.edu/~inderjit/public_papers/glued_mrr.pdf
        ...
        // filter
        // 8MNK + 18MN
        flop_count += 2 * N * chase_filtered_vecs * N;
    ...
    /*! Similar to `get_flops`, this counter return the average number of
       ...
       filter.
     */
    std::size_t get_lanczos_flops(std::size_t N, std::size_t lanczosIter,
    ...
    void start_clock(TimePtrs t) { start_points[t].push_back(getTimePoint()); }

    void set_early_locked_residuals(
        std::vector<chase::Base<T>> early_locked_residuals)
    {
        if (early_locked_residuals.size())
        {
            std::copy(early_locked_residuals.begin(),
                      early_locked_residuals.end(),
                      back_inserter(early_locked_residuals_));
        }
    }

    void end_clock(TimePtrs t) { end_points[t].push_back(getTimePoint()); }
     ...
     */
    void print(std::size_t N = 0, std::size_t lanczosIter = 0,
               std::size_t numLanczos = 0)
    {
        ...
        if (N != 0)
        {
            ...
            {
                stream << std::scientific << std::setprecision(3)
                       << static_cast<double>(lanczos_flops);
                all_values.push_back(stream.str());
                stream.str("");
            }
            stream << std::scientific << std::setprecision(3)
        ...
        if (1 || num)
        {
            ...
            {
                Base<T> avg =
                    std::accumulate(early_locked_residuals_.begin(),
                                    early_locked_residuals_.end(), 0.0);
                avg /= num;
                stream << std::scientific << std::setprecision(3)
            ...
            if (num)
            {
                Base<T> max = *std::max_element(early_locked_residuals_.begin(),
                                                early_locked_residuals_.end());
                stream << std::scientific << std::setprecision(3)
        ...
        printTable(output_names, all_values);
    }

private:
    inline std::chrono::duration<double>
    calculatePointTiming(TimePointType& start, TimePointType& stop)
    {
#ifdef HAS_CUDA
        float tmp_milisec = 0;
    ...
    void calculateTimings()
    {
        ...
        for (size_t it = 0; it < start_points.size(); ++it)
        {
            ...
            {
                timings[it] += calculatePointTiming(start_points[it][vecit],
                                                    end_points[it][vecit]);
            }
        }
    }
                    ...
                    const std::vector<std::string>& all_values)
    {
        ...
        {
            int max_width =
                std::max(output_names[i].size(), all_values[i].size());
...
{
public:
    PerformanceDecoratorChase(ChaseBase<T>* chase)
        : chase_(chase), perf_(!chase->isSym())
    {
    }
    ...
    bool checkSymmetryEasy() { return chase_->checkSymmetryEasy(); }

    bool isSym() { return chase_->isSym(); }

    bool checkPseudoHermicityEasy()
    {
        return chase_->checkPseudoHermicityEasy();
    }
    bool isPseudoHerm() { return chase_->isPseudoHerm(); }

    void symOrHermMatrix(char uplo) { chase_->symOrHermMatrix(uplo); }
