...
class ChASE_PerfData {
 public:
  ...
  void print_timings() {
    ...
    std::cout << " | All | Lanczos | Degrees | Filter | QR | RR | Resid | "
              << std::endl;

    std::cout << " | " << timings[TimePtrs::All] << " | "
              << timings[TimePtrs::Lanczos] << " | "
              << timings[TimePtrs::Degrees] << " | "
              << timings[TimePtrs::Filter] << " | " << timings[TimePtrs::Qr]
              << " | " << timings[TimePtrs::Rr] << " | "
              << timings[TimePtrs::Resids_Locking] << " | " << std::endl;
  }
