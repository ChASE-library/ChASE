...
template <typename T>
void
assertEqual(typename std::unordered_map<std::string, T>::iterator it,
            std::unordered_map<std::string, T> const& map, std::size_t& tests,
            std::size_t& fails)
{
...
void
assertEqual(typename std::unordered_map<std::string, T>::iterator it,
            std::unordered_map<std::string, T> const& map, T tolerance,
            std::size_t& tests, std::size_t& fails)
{
  auto rhs = map.at(it->first);
  if (std::abs((it->second - rhs) / std::max(it->second, rhs)) > tolerance) {
    fails++;
    std::cout << it->first << " fails comparison\t|" << it->second << " - "
              << rhs << "| > " << tolerance << std::endl;
  }
  tests++;
}
...
public:
  ...
  friend std::ostream& operator<<(std::ostream& os,
                                  const TestResultIteration& tr);

  TestResultIteration(){};

  void compareMembers(TestResultIteration& ref, std::size_t& tests,
                      std::size_t& fails)
  {
...
private:
  ...
  {
    ar& BOOST_SERIALIZATION_NVP(intMap) & BOOST_SERIALIZATION_NVP(doubleMap);
  }
};
...
public:
  ...
  TestResult(bool compare, std::string name, std::size_t n, std::size_t nev,
             std::size_t nex, std::size_t deg, double tol, char mode, char opt,
             bool sequence);
