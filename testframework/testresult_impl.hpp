...
void TestResult::loadAndCompare() {
  ...
  try {
    ...
    std::cout << ANSI_FG_BLACK << ANSI_BG_YELLOW << "File not found, writing"
              << std::endl
              << ANSI_RESET << ANSI_RESET
              << "Consider creating the test profile for the whole sequence"
              << std::endl;
...
void TestResult::reportResult(std::size_t tests, std::size_t fails) {
  ...
  std::cout << ANSI_FG_BLACK << "PASSED\t" << (tests - fails) << " / " << tests
            << ANSI_RESET << std::endl;
}
