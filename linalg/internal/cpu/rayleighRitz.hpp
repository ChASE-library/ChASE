...
namespace cpu
{
    ...
    */    
/*    template<typename T>
    void rayleighRitz(chase::matrix::QuasiHermitianMatrix<T>, std::size_t n, T *Q, std::size_t ldq, 
                    T * W, std::size_t ldw, Base<T> *ritzv, T *A = nullptr)
    {
        std::unique_ptr<T[]> ptr;
	...
	blasspp::t_gemm(CblasColMajor, CblasConjTrans, CblasNoTrans, n, n, k, &One
		Q, ldq, Q,ldq, &Zero, A, lda);
