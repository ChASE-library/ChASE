 ...
 * @param[out] V The output matrix (N x n), which will store the result of the
 * projection.
 ...
 * the computed ritz vectors.
 */
template <typename T>
void rayleighRitz(chase::matrix::Matrix<T>* H, std::size_t n, T* Q,
                  std::size_t ldq, T* V, std::size_t ldv, Base<T>* ritzv,
                  T* A = nullptr)
{
    std::size_t N = H->rows();
    std::size_t ldh = H->ld();
 ...
 * eigenvectors of a matrix by projecting the matrix onto a subspace defined by
 * a set of vectors (Q) and solving the eigenvalue problem for the reduced
 * matrix. The real parts of the computed Ritz values are stored in the `ritzv`
 * array, and the resulting right eigenvectors are stored in `V`.
 *
 * @tparam T Data type for the matrix (e.g., float, double, etc.).
 ...
 * 2. Computes A = V' * Q, where V' is the conjugate transpose of V.
 * 3. Solves the eigenvalue problem for A using LAPACK's `geev` function,
 * computing the real part of Ritz values in `ritzv`.
 * 4. Computes the final approximation to the eigenvectors by multiplying Q with
 * the computed ritz vectors.
 */
template <typename T>
void rayleighRitz(chase::matrix::PseudoHermitianMatrix<T>* H, std::size_t n,
                  T* Q, std::size_t ldq, T* V, std::size_t ldv, Base<T>* ritzv,
                  T* A = nullptr)
{
    ...
    blaspp::t_gemm(CblasColMajor, CblasConjTrans, CblasNoTrans, n, n, N, &One,
                   Q, ldq, V, ldv, &Zero, W, n); // A = Qr^* T

    // Performs Q_2^T Q_2 for the construction of the dual basis, Q_2 is the
    // lower part of Q
    blaspp::t_gemm(CblasColMajor, CblasConjTrans, CblasNoTrans, n, n, k,
                   &NegativeTwo, Q + k, ldq, Q + k, ldq, &Zero, M,
                   n); // M = -2 Qr_2^* Qr_2
    ...
    // Flip the sign of the lower part of V to emulate the multiplication H' *
    // Ql
    chase::linalg::internal::cpu::flipLowerHalfMatrixSign(N, n, V,
                                                          ldv); // flip T
    ...
    lapackpp::t_geev(LAPACK_COL_MAJOR, 'V', n, A, n, ritzv, ritzvi.data(), W,
                     n);
 ...
 */
template <typename T>
void rayleighRitz_v2(chase::matrix::PseudoHermitianMatrix<T>* H, std::size_t n,
                     T* Q, std::size_t ldq, T* V, std::size_t ldv,
                     Base<T>* ritzv, T* A = nullptr)
{
    ...
    blaspp::t_gemm(CblasColMajor, CblasConjTrans, CblasNoTrans, n, n, k,
                   &NegativeTwo, Q + k, ldq, Q + k, ldq, &One, M,
                   n); // M = I -2 Q_2^* Q_2
