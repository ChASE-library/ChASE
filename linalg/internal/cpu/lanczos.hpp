...
namespace cpu
{
     ...
     */    
    template<typename T>
    void lanczos(std::size_t M, std::size_t numvec, chase::matrix::Matrix<T> * H, T *V, std::size_t ldv, 
	...
	//for(auto i = 0; i < N; i++) std::cout << Sv.data()[i] << " " << Sv.data()[i + N] << std::endl;

	chase::linalg::internal::cpu::flipLowerHalfMatrixSign(N,numvec, Sv.data(), N);
	...
	for(auto i = 0; i < numvec; i++)
	    ...
	    for(auto i = 0; i < numvec; i++)
	    {
		alpha[i] = blaspp::t_dot(N,v_2.data() + i * N, 1, Sv.data() + i * N, 1);
	    }
