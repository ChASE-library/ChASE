...
void cpu_mpi::lanczos_dispatch(
    std::size_t M, std::size_t numvec, MatrixType& H, InputMultiVectorType& V,
    ...
    chase::Base<typename MatrixType::value_type>* ritzV)
{
    ...
    {
        cpu_mpi::pseudo_hermitian_lanczos(M, numvec, H, V, upperb, ritzv, Tau,
                                          ritzV);
    }
    else
    {
        cpu_mpi::lanczos(M, numvec, H, V, upperb, ritzv, Tau, ritzV);
    }
}
 ...
 */
template <typename MatrixType, typename InputMultiVectorType>
void cpu_mpi::lanczos(std::size_t M, std::size_t numvec, MatrixType& H,
                      ...
                      chase::Base<typename MatrixType::value_type>* ritzV)
{
    ...
    chase::linalg::lapackpp::t_lacpy('A', v_1.l_rows(), numvec, V.l_data(),
                                     V.l_ld(), v_1.l_data(), v_1.l_ld());

    for (auto i = 0; i < numvec; i++)
    {
        real_alpha[i] = chase::linalg::blaspp::t_norm_p2(
            v_1.l_rows(), v_1.l_data() + i * v_1.l_ld());
    }
    ...
    for (auto i = 0; i < numvec; i++)
    {
        chase::linalg::blaspp::t_scal(v_1.l_rows(), &alpha[i],
    ...
    for (std::size_t k = 0; k < M; k = k + 1)
    {
        ...
        for (auto i = 0; i < numvec; i++)
        {
            alpha[i] = chase::linalg::blaspp::t_dot(
                v_1.l_rows(), v_1.l_data() + i * v_1.l_ld(), 1,
        ...
        for (auto i = 0; i < numvec; i++)
        {
            chase::linalg::blaspp::t_axpy(v_1.l_rows(), &alpha[i],
                                          v_1.l_data() + i * v_1.l_ld(), 1,
                                          v_2.l_data() + i * v_2.l_ld(), 1);
            alpha[i] = -alpha[i];
        }
        ...
        if (k > 0)
        {
            ...
            {
                beta[i] = T(-r_beta[i]);
                chase::linalg::blaspp::t_axpy(v_0.l_rows(), &beta[i],
        ...
        for (auto i = 0; i < numvec; i++)
        {
            r_beta[i] = chase::linalg::blaspp::t_norm_p2(
                v_2.l_rows(), v_2.l_data() + i * v_2.l_ld());
        }
        ...
        for (auto i = 0; i < numvec; i++)
        {
            chase::linalg::blaspp::t_scal(v_2.l_rows(), &beta[i],
                                          v_2.l_data() + i * v_2.l_ld(), 1);
        }
    ...
    lapackpp::t_lacpy('A', v_1.l_rows(), numvec, v_1.l_data(), v_1.l_ld(),
                      V.l_data(), V.l_ld());
 ...
 */
template <typename MatrixType, typename InputMultiVectorType>
void cpu_mpi::lanczos(std::size_t M, MatrixType& H, InputMultiVectorType& V,
                      chase::Base<typename MatrixType::value_type>* upperb)
{
    using T = typename MatrixType::value_type;
    using ResultMultiVectorType =
    ...
    chase::linalg::lapackpp::t_lacpy('A', v_1.l_rows(), 1, V.l_data(), V.l_ld(),
                                     v_1.l_data(), v_1.l_ld());
    ...
    for (std::size_t k = 0; k < M; k = k + 1)
    {
        ...
        alpha = chase::linalg::blaspp::t_dot(v_1.l_rows(), v_1.l_data(), 1,
                                             v_2.l_data(), 1);
        alpha = -alpha;
        ...
        chase::linalg::blaspp::t_axpy(v_1.l_rows(), &alpha, v_1.l_data(), 1,
                                      v_2.l_data(), 1);
        alpha = -alpha;
        ...
        {
            beta = T(-r_beta);
            chase::linalg::blaspp::t_axpy(v_0.l_rows(), &beta, v_0.l_data(), 1,
                                          v_2.l_data(), 1);
        }
