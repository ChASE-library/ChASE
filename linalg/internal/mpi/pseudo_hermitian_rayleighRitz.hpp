...
void cpu_mpi::pseudo_hermitian_rayleighRitz(
                                       ...
                                       chase::platform::CPU>* A)
{
    ...
    if (A == nullptr)
    {
        ...
        A_ptr = std::make_unique<
            ...
            subSize, 3 * subSize, V1.getMpiGrid_shared_ptr());
        A = A_ptr.get();
    }
    ...
    chase::linalg::blaspp::t_gemm(
        CblasColMajor, CblasConjTrans, CblasNoTrans, subSize, subSize,
        W2.l_rows(), &One, W2.l_data() + offset * W2.l_ld(), W2.l_ld(),
    ...
    chase::linalg::internal::cpu_mpi::flipLowerHalfMatrixSign(V2, offset,
                                                              subSize);

    chase::linalg::blaspp::t_gemm(
        CblasColMajor, CblasConjTrans, CblasNoTrans, subSize, subSize,
        V2.l_rows(), &One, V2.l_data() + offset * V2.l_ld(), V2.l_ld(),
    ...
    blaspp::t_gemm(CblasColMajor, CblasNoTrans, CblasNoTrans, subSize, subSize,
                   subSize, &NegOne, M, subSize, W, subSize, &Zero, A->l_data(),
                   subSize); // A = (Diag(M) - M) * A

    chase::linalg::internal::cpu_mpi::flipLowerHalfMatrixSign(W1, offset,
                                                              subSize);
    ...
    chase::linalg::blaspp::t_axpy(subSize * subSize, &One, M, 1, A->l_data(),
                                  1);
    ...
    if constexpr (0 && std::is_same<T, std::complex<float>>::value)
    {
        ...
        lapackpp::t_geev(LAPACK_COL_MAJOR, 'V', subSize, A_d->l_data(), subSize,
                         ptx_double.data(), ptx_imag_double.data(), W_d,
                         subSize);

        // Convert results back to single precision
        std::transform(ptx_double.begin(), ptx_double.end(), ritzv + offset,
                       [](double val) { return static_cast<float>(val); });

        // Copy W back to single precision
        std::transform(W_d, W_d + subSize * subSize, W,
    ...
    {
        lapackpp::t_geev(LAPACK_COL_MAJOR, 'V', subSize, A->l_data(), subSize,
                         ritzv + offset, ritzvi.data(), W, subSize);
    }
    ...
    std::sort(indices.begin(), indices.end(),
              [&sorted_ritzv](std::size_t i1, std::size_t i2)
              { return sorted_ritzv[i1] < sorted_ritzv[i2]; });
    ...
    chase::linalg::blaspp::t_gemm(
        ...
        subSize, &One, V2.l_data() + offset * V2.l_ld(), V2.l_ld(), W, subSize,
 ...
 */
template <typename MatrixType, typename InputMultiVectorType>
void cpu_mpi::pseudo_hermitian_rayleighRitz_v2(
    MatrixType& H, InputMultiVectorType& V1, InputMultiVectorType& V2,
    typename ResultMultiVectorType<MatrixType, InputMultiVectorType>::type& W1,
    typename ResultMultiVectorType<MatrixType, InputMultiVectorType>::type& W2,
    chase::Base<typename MatrixType::value_type>* ritzv, std::size_t offset,
                                       ...
                                       chase::platform::CPU>* A)
{
    ...
    if (A == nullptr)
    {
        ...
        A_ptr = std::make_unique<
            chase::distMatrix::RedundantMatrix<T, chase::platform::CPU>>(
            subSize, 2 * subSize, V1.getMpiGrid_shared_ptr());
        A = A_ptr.get();
    }
    ...
    chase::linalg::internal::cpu_mpi::flipLowerHalfMatrixSign(V1, offset,
                                                              subSize);
    ...
    // Create the inverse of the Hermitian Rayleigh Quotient by two successive
    // trsm
    blaspp::t_trsm('L', 'L', 'N', 'N', subSize, subSize, &One, A->l_data(),
                   subSize, M, subSize);
    ...
    {
        // Note: For CPU version, we need to work with M which is a pointer into
        // A We'll need to create a temporary matrix or use the API differently
        ...
        lapackpp::t_heevd(LAPACK_COL_MAJOR, 'V', 'L', subSize, M_d, subSize,
                          ritzv_double.data());

        // Convert ritzv back to single precision
        std::transform(ritzv_double.begin(), ritzv_double.end(), ritzv + offset,
                       [](double val) { return static_cast<float>(val); });
    ...
    {
#endif
        lapackpp::t_heevd(LAPACK_COL_MAJOR, 'V', 'L', subSize, M, subSize,
                          ritzv + offset);
#ifdef RR_DOUBLE_PRECISION
    }
#endif
    ...
    for (auto idx = 0; idx < subSize; idx++)
    {
        if (ritzv[idx + offset] == 0.0)
        {
            throw std::runtime_error(
                "HEEVD failed in Pseudo-Hermitian RayleighRitz: zero "
                "eigenvalue encountered at index " +
                std::to_string(idx));
        }
