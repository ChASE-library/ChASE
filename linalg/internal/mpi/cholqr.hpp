 ...
 *
 * Computes a Cholesky factorization-based QR decomposition for the input matrix
 * `V` in an MPI-distributed setting. If `A` is not provided, it is allocated
 * internally.
 *
 ...
 * @param A Optional pointer to an allocated `n x n` matrix used in the Cholesky
 * factorization.
 *
 * @return int Returns 0 on success; non-zero on failure.
 */
template <typename T>
int cpu_mpi::cholQR1(std::size_t m, std::size_t n, T* V, int ldv, MPI_Comm comm,
                     T* A)
{
    T one = T(1.0);
    T zero = T(0.0);
    ...
    MPI_Allreduce(MPI_IN_PLACE, A, n * n, chase::mpi::getMPI_Type<T>(), MPI_SUM,
                  comm);
 ...
 *
 * Computes a Cholesky factorization-based QR decomposition with two Cholesky
 * steps for the input matrix `V` in an MPI-distributed setting. If `A` is not
 ...
 */
template <typename T>
int cpu_mpi::cholQR2(std::size_t m, std::size_t n, T* V, int ldv, MPI_Comm comm,
                     T* A)
{
    T one = T(1.0);
    T zero = T(0.0);
    ...
    else
    {
        ...
        MPI_Allreduce(MPI_IN_PLACE, A, n * n, chase::mpi::getMPI_Type<T>(),
                      MPI_SUM, comm);
        info = lapackpp::t_potrf('U', n, A, n);
        blaspp::t_trsm('R', 'U', 'N', 'N', m, n, &one, A, n, V, ldv);
#ifdef CHASE_OUTPUT
 ...
 * type handling. The use of MPI ensures that the computation is parallelized
 ...
 * @param A Optional pointer to an allocated matrix used in the Cholesky
 * factorization.
 *
                     ...
                     typename InputMultiVectorType::value_type* A)
{
    ...
    blaspp::t_syherk('U', 'C', V.l_cols(), V.l_rows(), &one, V.l_data(),
                     V.l_ld(), &zero, A, V.l_cols());

    MPI_Allreduce(MPI_IN_PLACE, A, V.l_cols() * V.l_cols(),
                  chase::mpi::getMPI_Type<T>(), MPI_SUM,
                  V.getMpiGrid()->get_col_comm());

    info = lapackpp::t_potrf('U', V.l_cols(), A, V.l_cols());
    ...
    {
        blaspp::t_trsm('R', 'U', 'N', 'N', V.l_rows(), V.l_cols(), &one, A,
 ...
 * multi-vector `V` using BLAS and LAPACK on the CPU. It is designed to work
 * with multi-vector input types where `V` can be a matrix or a vector, and the
 * decomposition is performed in parallel on multiple CPUs. The function also
 * allows for memory optimization through an optional workspace buffer for
 ...
 * compatible with BLAS and LAPACK. The input matrix must be stored in
 * column-major format.
 *
 * @note This function supports mixed precision when ENABLE_MIXED_PRECISION is
 * defined. For double precision types, it can use single precision for the
 * first Cholesky QR step to improve performance while maintaining accuracy.
 */
template <typename InputMultiVectorType>
 ...
 *
 * Computes a shifted Cholesky factorization-based QR decomposition with two
 * Cholesky steps for the input matrix `V` in an MPI-distributed setting. If `A`
 * is not provided, it is allocated internally.
 *
 * @tparam T The data type of the matrix elements.
 *
 ...
 */
template <typename T>
int cpu_mpi::shiftedcholQR2(std::size_t N, std::size_t m, std::size_t n, T* V,
                            int ldv, MPI_Comm comm, T* A)
{
    Base<T> shift;
    T one = T(1.0);
    T zero = T(0.0);
...
/**
 * @brief Computes the Householder QR decomposition for a distributed
 * multi-vector.
 *
 * This function performs a QR factorization using Householder transformations
 * on the input multi-vector `V`. Requires ScaLAPACK to be available for
 * distributed QR computations.
 *
 * @tparam InputMultiVectorType The type of the distributed multi-vector,
...
void cpu_mpi::houseHoulderQR(InputMultiVectorType& V)
{
    ...
    chase::linalg::scalapackpp::t_pgeqrf(V.g_rows(), V.g_cols(), V.l_data(),
...
/**
 * @brief Computes the condition number of a distributed matrix using ScaLAPACK
 * SVD.
 *
 * This function computes the condition number of a distributed matrix \( V \)
 * using the singular value decomposition (SVD). The condition number is defined
 * as the ratio of the largest to the smallest singular value: cond(V) = σ_max /
 * σ_min. It first allocates and transfers the data to the CPU if not already
 * allocated, performs the SVD using ScaLAPACK routines, and then computes the
 * condition number from the singular values.
 *
 * @tparam InputMultiVectorType The type of the input multi-vector (e.g., a
 * distributed matrix).
 *
 * @param[in] V The input distributed matrix. The matrix should be in
 * column-major order. Note: the matrix content will be destroyed during SVD
 * computation.
 *
 * @return chase::Base<typename InputMultiVectorType::value_type> The condition
 * number of the matrix.
 *
 * @throws std::runtime_error If ScaLAPACK is not available (i.e., for ChASE-MPI
 * builds without ScaLAPACK support).
 *
 * @note This function requires ScaLAPACK for distributed SVD. If ScaLAPACK is
 * not available, a runtime error will be thrown.
 * @note The input matrix V will be destroyed during the SVD computation. Make a
 * copy if the original matrix needs to be preserved.
 * @note If the matrix is rank-deficient (smallest singular value is zero or
 * near machine precision), the condition number will be very large or infinite.
 *
 * @par ScaLAPACK Functions Used:
 *   - `t_pgesvd`: Computes the singular value decomposition of the matrix.
 */
template <typename InputMultiVectorType>
chase::Base<typename InputMultiVectorType::value_type>
cpu_mpi::computeConditionNumber(InputMultiVectorType& V)
{
    using T = typename InputMultiVectorType::value_type;
    ...
    chase::linalg::scalapackpp::t_pgesvd(
        jobu, jobvt, m, n, V.l_data(), one, one, desc_a, s.data(), &dummy_u,
        one, one, desc_u,             // U not computed
...
#else
    throw std::runtime_error("For ChASE-MPI, computing condition number "
                             "requires ScaLAPACK, which is not detected\n");
#endif
}
