...
void cpu_mpi::pseudo_hermitian_lanczos(
    ...
    chase::Base<typename MatrixType::value_type>* ritzV)
{
    ...
    chase::linalg::internal::cpu_mpi::MatrixMultiplyMultiVectors(&One, H, v_1,
                                                                 &Zero, v_w);

    v_w.redistributeImpl(&v_2);

    chase::linalg::lapackpp::t_lacpy('A', Sv.l_rows(), numvec, v_2.l_data(),
    ...
    for (auto i = 0; i < numvec; i++)
    {
        chase::linalg::blaspp::t_scal(v_1.l_rows(), &beta[i],
                                      v_1.l_data() + i * v_1.l_ld(), 1);
    }
    ...
    for (std::size_t k = 0; k < M; k = k + 1)
    {
        ...
        for (auto i = 0; i < numvec; i++)
        {
            alpha[i] = chase::linalg::blaspp::t_dot(
                v_2.l_rows(), v_2.l_data() + i * v_2.l_ld(), 1,
                Sv.l_data() + i * Sv.l_ld(), 1);
        }
        ...
        for (auto i = 0; i < numvec; i++)
        {
            beta[i] = chase::linalg::blaspp::t_dot(
    ...
    chase::Base<typename MatrixType::value_type>* upperb)
{
    ...
    chase::linalg::lapackpp::t_lacpy('A', Sv.l_rows(), 1, v_2.l_data(),
                                     v_2.l_ld(), Sv.l_data(), Sv.l_ld());

    chase::linalg::internal::cpu_mpi::flipLowerHalfMatrixSign(Sv);

    beta = chase::linalg::blaspp::t_dot(v_1.l_rows(), v_1.l_data(), 1,
                                        Sv.l_data(), 1);
    ...
    chase::linalg::blaspp::t_scal(v_1.l_rows(), &beta, v_1.l_data(), 1);
    ...
    for (std::size_t k = 0; k < M; k = k + 1)
    {
        ...
        alpha = chase::linalg::blaspp::t_dot(v_2.l_rows(), v_2.l_data(), 1,
                                             Sv.l_data(), 1);
