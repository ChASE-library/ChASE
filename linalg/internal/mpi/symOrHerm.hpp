...
bool cpu_mpi::checkSymmetryEasy(MatrixType& H)
{
    ...
    MatrixMultiplyMultiVectors(&One, H, v, &Zero, u);

    MatrixMultiplyMultiVectors(&One, H, v_2, &Zero, uT);
...
/**
 * @brief Converts a BlockBlock matrix to its Hermitian or symmetric form based
 * on the given `uplo` argument.
 *
 ...
 */
template <typename T>
void cpu_mpi::symOrHermMatrix(char uplo,
...
#pragma omp parallel for
    ...
    chase::linalg::scalapackpp::t_ptranc(H.g_rows(), H.g_cols(), One,
                                         H.l_data(), one, one, desc, Zero,
...
/**
 * @brief Converts a BlockCyclic matrix to its Hermitian or symmetric form based
 * on the given `uplo` argument.
 *
 * This function modifies the matrix \( H \) in-place such that it becomes
 * symmetric or Hermitian, depending on the value of the `uplo` parameter.
            ...
            for (std::size_t i = 0; i < mblocks; i++)
            {
                for (std::size_t q = 0; q < n_contiguous_lens[j]; q++)
                {
                    for (std::size_t p = 0; p < m_contiguous_lens[i]; p++)
                    {
                        ...
                        {
                            H.l_data()[(q + n_contiguous_local_offs[j]) *
                                           H.l_ld() +
                                       p + m_contiguous_local_offs[i]] *=
                                T(0.5);
                        }
                        else if (q + n_contiguous_global_offs[j] <
                                 p + m_contiguous_global_offs[i])
                        {
                            H.l_data()[(q + n_contiguous_local_offs[j]) *
                                           H.l_ld() +
                                       p + m_contiguous_local_offs[i]] = T(0.0);
                        }
                    }
                }
            }
