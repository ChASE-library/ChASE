 ...
 *
 * This function iterates over the local rows and columns of the distributed
 * matrix `H` and adds a given shift value to the diagonal elements (where row
 * index equals column index).
 *
 ...
 */
template <typename T>
void cpu_mpi::shiftDiagonal(
    chase::distMatrix::BlockBlockMatrix<T, chase::platform::CPU>& H, T shift)
{
 ...
 *
 * This function iterates over the matrix blocks, identifying diagonal elements
 * based on global and local offsets, and adds a specified shift value to these
 * elements.
 *
 * @tparam T The data type of the matrix elements (e.g., `float`, `double`).
...
template <typename T>
void cpu_mpi::shiftDiagonal(
    chase::distMatrix::BlockCyclicMatrix<T, chase::platform::CPU>& H, T shift)
{
        ...
        for (std::size_t i = 0; i < mblocks; i++)
        {
            for (std::size_t q = 0; q < n_contiguous_lens[j]; q++)
            {
                for (std::size_t p = 0; p < m_contiguous_lens[i]; p++)
                {
                    ...
                    {
                        H.l_data()[(q + n_contiguous_local_offs[j]) * H.l_ld() +
                                   p + m_contiguous_local_offs[i]] += shift;
                    }
                }
            }
        }
