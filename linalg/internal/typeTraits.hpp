 ...
 * 
 * This group contains helper traits and structs for deducing the correct communicator type and result multi-vector
 * types based on matrix and multi-vector types in the distributed matrix and multi-vector system.
 */

/**
 * @ingroup MultiVectorHelperTraits
 * @brief Helper struct to deduce the communicator type from the multi-vector type.
 * 
 * This struct is used to extract the communicator type (`CommType`) from a given multi-vector type, allowing
 * the system to handle various communicator types in a flexible manner.
 */ 
 ...
 * 
 * Specialization of `ExtractCommType` for the `DistMultiVector1D` type, which extracts the communicator type
 * from the provided multi-vector type.
 */
 ...
 * 
 * Specialization of `ExtractCommType` for the `DistMultiVectorBlockCyclic1D` type, which extracts the communicator
 * type from the provided multi-vector type.
 */
template <typename T, chase::distMultiVector::CommunicatorType CommType, typename Platform>
 ...
 * 
 * This struct provides a way to "flip" the communicator type (from row to column and vice versa) for multi-vectors
...
/**
 * @ingroup MultiVectorHelperTraits
 * @brief Helper trait to deduce the result multi-vector type based on input matrix and multi-vector types.
 * 
 * This trait is used to deduce the correct result multi-vector type after performing operations involving matrix and
 * multi-vector types. It handles communicator type flipping where necessary.
 */
 ...
 * 
 * This specialization defines the result multi-vector type for operations involving a `BlockBlockMatrix` and a
...
/**
 * @ingroup MultiVectorHelperTraits
 * @brief Specialization for `BlockCyclicMatrix` and `DistMultiVectorBlockCyclic1D`.
 * 
 * This specialization defines the result multi-vector type for operations involving a `BlockCyclicMatrix` and a
 * `DistMultiVectorBlockCyclic1D` multi-vector type. It flips the communicator type as part of the deduction.
 */
template <typename T, chase::distMultiVector::CommunicatorType CommType, typename Platform>
 ...
 * multi-vector communicator for the column-wise operations.
 */
 ...
 * 
 * This specialization defines the column multi-vector type for the `BlockBlockMatrix` type, which corresponds to a
 ...
 * 
 * This specialization defines the column multi-vector type for the `BlockCyclicMatrix` type, which corresponds to a
 ...
 * multi-vector communicator for the row-wise operations.
 */
template <typename MatrixType>
 ...
 * @brief Specialization for `BlockBlockMatrix`.
 * 
 * This specialization defines the row multi-vector type for the `BlockBlockMatrix` type, which corresponds to a
 ...
 * @brief Specialization for `BlockCyclicMatrix`.
 * 
 * This specialization defines the row multi-vector type for the `BlockCyclicMatrix` type, which corresponds to a
