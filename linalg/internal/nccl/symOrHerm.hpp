...
bool cuda_nccl::checkSymmetryEasy(cublasHandle_t cublas_handle, MatrixType& H)
{
    ...
    chase::linalg::internal::cuda::chase_rand_normal(
        seed, states_, v.l_data(), v.l_ld() * v.l_cols(), (cudaStream_t)0);

    v.redistributeImpl(&v_2);

    chase::linalg::internal::cuda_nccl::MatrixMultiplyMultiVectors(
        cublas_handle, &One, H, v, &Zero, u);

    chase::linalg::internal::cuda_nccl::MatrixMultiplyMultiVectors(
        cublas_handle, &One, H, v_2, &Zero, uT);
...
// this function now assumes the hermitian matrix is provided on CPU
template <typename T>
void cuda_nccl::symOrHermMatrix(
    char uplo, chase::distMatrix::BlockBlockMatrix<T, chase::platform::GPU>& H)
{
#ifdef HAS_SCALAPACK
...
template <typename T>
void cuda_nccl::symOrHermMatrix(
    char uplo, chase::distMatrix::BlockCyclicMatrix<T, chase::platform::GPU>& H)
{
#ifdef HAS_SCALAPACK
    std::size_t* desc = H.scalapack_descriptor_init();
            ...
            for (std::size_t i = 0; i < mblocks; i++)
            {
                for (std::size_t q = 0; q < n_contiguous_lens[j]; q++)
                {
                    for (std::size_t p = 0; p < m_contiguous_lens[i]; p++)
                    {
                        ...
                        {
                            H.cpu_data()[(q + n_contiguous_local_offs[j]) *
                                             H.cpu_ld() +
