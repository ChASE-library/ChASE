...
void cuda_nccl::pseudo_hermitian_rayleighRitz(
                                       ...
                                       chase::platform::GPU>* A)
{
    ...
    CHECK_NCCL_ERROR(chase::nccl::ncclAllReduceWrapper<T>(
        W, W, subSize * subSize, ncclSum,
        A->getMpiGrid()->get_nccl_row_comm()));

    chase::linalg::internal::cuda_nccl::flipLowerHalfMatrixSign(V2, offset,
                                                                subSize);
 ...
 */
template <typename MatrixType, typename InputMultiVectorType>
void cuda_nccl::pseudo_hermitian_rayleighRitz_v2(
    cublasHandle_t cublas_handle, cusolverDnHandle_t cusolver_handle,
    cusolverDnParams_t params, MatrixType& H, InputMultiVectorType& V1,
    InputMultiVectorType& V2,
    typename ResultMultiVectorType<MatrixType, InputMultiVectorType>::type& W1,
                                       ...
                                       chase::platform::GPU>* A)
{
    ...
    // Perform the MPI_Allreduce to sum up the results
    // chase::linalg::internal::cuda::extractUpperTriangular(A->l_data(),
    // subSize, workspace, subSize);
    // CHECK_NCCL_ERROR(chase::nccl::ncclAllReduceWrapper<T>(workspace,
    // workspace, upperTriangularSize, ncclSum,
    // A->getMpiGrid()->get_nccl_row_comm()));
    ...
    // chase::linalg::internal::cuda::extractUpperTriangular(M, subSize,
    // workspace, subSize);
    // CHECK_NCCL_ERROR(chase::nccl::ncclAllReduceWrapper<T>(workspace,
    // workspace, upperTriangularSize, ncclSum,
    // A->getMpiGrid()->get_nccl_col_comm()));
