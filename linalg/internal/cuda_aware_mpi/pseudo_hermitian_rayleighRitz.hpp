...
{
/**
 * @brief Performs the Rayleigh-Ritz procedure, reducing the matrix H using the
 * basis vectors in V1 and V2, and storing the results in W1, W2, and the
 * eigenvalues in ritzv.
 *
 * This function computes the distributed matrix multiplication and reduction
 * required for the Rayleigh-Ritz procedure. If matrix A is provided, it uses
 * that as a workspace. Otherwise, a temporary matrix is created.
 *
 ...
 * @param ritzv Pointer to an array where computed eigenvalues of the reduced
 * matrix will be stored.
 * @param offset The starting offset in V1, V2, W1, and W2 for submatrix
 * processing.
 * @param subSize Size of the submatrix used in reduction.
 * @param A Optional workspace matrix for storing intermediate results; if
 * nullptr, a temporary matrix is created.
 *
 * @throws std::invalid_argument if ritzv is a nullptr.
 */
template <typename MatrixType, typename InputMultiVectorType>
void cuda_mpi::pseudo_hermitian_rayleighRitz(
    ...
    typename MatrixType::value_type* d_workspace, int d_lwork,
    typename MatrixType::value_type* h_workspace, int h_lwork,
                                       ...
                                       chase::platform::GPU>* A)
{
    ...
    if (A == nullptr)
    {
        ...
        A_ptr = std::make_unique<
            ...
            3 * subSize, subSize, V1.getMpiGrid_shared_ptr());
        A = A_ptr.get();
    }
    ...
    if (d_workspace == nullptr ||
        h_workspace == nullptr) // To update once Xgeev is plugged in
    {
        ...
        CHECK_CUSOLVER_ERROR(
            chase::linalg::cusolverpp::cusolverDnTgeev_bufferSize(
                cusolver_handle, params, CUSOLVER_EIG_MODE_NOVECTOR,
                CUSOLVER_EIG_MODE_VECTOR, subSize, A->l_data(), subSize,
                ritzv_complex, NULL, 1, V1.l_data() + offset * V1.l_ld(),
                V1.l_ld(), &temp_d_lwork, &temp_h_lwork));
        if (d_workspace == nullptr)
        {
            ...
            CHECK_CUDA_ERROR(
                cudaMalloc((void**)&d_workspace, sizeof(T) * d_lwork));
        }
    ...
    chase::distMatrix::RedundantMatrix<
        chase::Base<typename MatrixType::value_type>, chase::platform::GPU>
        diag(1, subSize, V1.getMpiGrid_shared_ptr());
    ...
    chase::linalg::internal::cuda_mpi::
        MatrixMultiplyMultiVectorsAndRedistribute(cublas_handle, H, V1, W1, V2,
                                                  W2, offset, subSize);
    ...
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        ...
        W1.l_data() + offset * W1.l_ld(), W1.l_ld(), &Zero, W, subSize));

    // Perform the MPI_Allreduce to sum up the results
    CHECK_CUDA_ERROR(cudaDeviceSynchronize());
    MPI_Allreduce(MPI_IN_PLACE, W, subSize * subSize,
                  chase::mpi::getMPI_Type<T>(), MPI_SUM,
                  A->getMpiGrid()->get_row_comm());
    CHECK_CUDA_ERROR(cudaDeviceSynchronize());

    chase::linalg::internal::cuda_mpi::flipLowerHalfMatrixSign(V2, offset,
                                                               subSize);

    CHECK_CUDA_ERROR(cudaDeviceSynchronize());
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_C, CUBLAS_OP_N, subSize, subSize, V2.l_rows(),
        &One, V2.l_data() + offset * V2.l_ld(), V2.l_ld(),
        V1.l_data() + offset * V1.l_ld(), V1.l_ld(), &Zero, M, subSize));

    CHECK_CUDA_ERROR(cudaDeviceSynchronize());
    MPI_Allreduce(MPI_IN_PLACE, M, subSize * subSize,
                  chase::mpi::getMPI_Type<T>(), MPI_SUM,
                  A->getMpiGrid()->get_col_comm());
    CHECK_CUDA_ERROR(cudaDeviceSynchronize());

    chase::linalg::internal::cuda::chase_plus_inverse_diagonal(
        M, subSize, subSize, real_Zero, diag.l_data(), usedStream);

    chase::linalg::internal::cuda::chase_set_diagonal(M, subSize, subSize, Zero,
                                                      usedStream);

    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, subSize, subSize, subSize,
        &NegOne, M, subSize, W, subSize, &Zero, A->l_data(),
        subSize)); // A = (Diag(M) - M) * A

    chase::linalg::internal::cuda_mpi::flipLowerHalfMatrixSign(W1, offset,
                                                               subSize);
    ...
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_C, CUBLAS_OP_N, subSize, subSize, W2.l_rows(),
        &One, W2.l_data() + offset * W2.l_ld(), W2.l_ld(),
        W1.l_data() + offset * W1.l_ld(), W1.l_ld(), &Zero, M, subSize));
    ...
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTaxpy(
        cublas_handle, subSize * subSize, &One, M, 1, A->l_data(), 1));

    chase::linalg::internal::cuda::chase_scale_rows_matrix(
        A->l_data(), subSize, subSize, subSize, diag.l_data(), usedStream);
    ...
    if constexpr (0 && std::is_same<T, std::complex<float>>::value)
    {
        ...
        std::complex<double>* ritzv_complex_d =
            A_d->l_data() + 1 * subSize * subSize;

        CHECK_CUSOLVER_ERROR(chase::linalg::cusolverpp::cusolverDnTgeev(
            cusolver_handle, params, CUSOLVER_EIG_MODE_NOVECTOR,
            CUSOLVER_EIG_MODE_VECTOR, subSize, A_d->l_data(), subSize,
            ritzv_complex_d, NULL, 1, W_d, subSize,
            reinterpret_cast<std::complex<double>*>(d_workspace), d_lwork,
            reinterpret_cast<std::complex<double>*>(h_workspace), h_lwork,
            devInfo));
        ...
        {
            cudaMemcpy(ritzvs_cmplex_cpu.data(), ritzv_complex,
                       subSize * sizeof(T), cudaMemcpyDeviceToHost);
        ...
        {
            CHECK_CUDA_ERROR(cudaMemcpy(ptx, ritzv_complex,
    ...
    else
    {
        CHECK_CUSOLVER_ERROR(chase::linalg::cusolverpp::cusolverDnTgeev(
            cusolver_handle, params, CUSOLVER_EIG_MODE_NOVECTOR,
            CUSOLVER_EIG_MODE_VECTOR, subSize, A->l_data(), subSize,
            ritzv_complex, NULL, 1, W, subSize, d_workspace, d_lwork,
            h_workspace, h_lwork, devInfo));
        ...
        // std::endl; thrust::device_vector<int> indices(n); //Does not compile,
        // returns unimplemented on this system...
        if constexpr (std::is_same<T, std::complex<chase::Base<T>>>::value)
        {
            cudaMemcpy(ritzvs_cmplex_cpu.data(), ritzv_complex,
                       subSize * sizeof(T), cudaMemcpyDeviceToHost);
    ...
    if constexpr (0 && std::is_same<T, std::complex<float>>::value)
    {
        ...
        std::transform(A->cpu_data(), A->cpu_data() + subSize * subSize,
                       A_double.begin(), [](const T& val)
                       { return static_cast<std::complex<double>>(val); });

        // Compute eigenvalues and eigenvectors in double precision
        lapackpp::t_geev(LAPACK_COL_MAJOR, 'V', subSize, A_double.data(),
                         subSize, ptx_double.data(), ptx_imag_double.data(),
                         W_cpu_double.data(), subSize);

        // Convert results back to single precision
        std::transform(ptx_double.begin(), ptx_double.end(), ptx,
                       [](double val) { return static_cast<float>(val); });

        // Convert W and A back to single precision
        std::transform(W_cpu_double.begin(), W_cpu_double.end(), W_cpu,
                       [](const std::complex<double>& val)
                       { return static_cast<std::complex<float>>(val); });

        std::transform(A_double.begin(), A_double.end(), A->cpu_data(),
                       [](const std::complex<double>& val)
                       { return static_cast<std::complex<float>>(val); });
    ...
    {
        std::vector<chase::Base<T>> ptx_imag =
            std::vector<chase::Base<T>>(subSize, chase::Base<T>(0.0));

        A->D2H();

        T* W_cpu = A->cpu_data() + 2 * subSize * subSize;

        // Compute the eigenpairs of the non-hermitian rayleigh quotient on the
        // CPU
        lapackpp::t_geev(LAPACK_COL_MAJOR, 'V', subSize, A->cpu_data(), subSize,
                         ptx, ptx_imag.data(), W_cpu, subSize);
    ...
    std::sort(indices.begin(), indices.end(),
              [&ptx](std::size_t i1, std::size_t i2)
              { return ptx[i1] < ptx[i2]; });
    ...
    for (std::size_t i = 0; i < subSize; ++i)
    {
        ...
        CHECK_CUDA_ERROR(
            cudaMemcpy(d_sorted_W + i * subSize, W + indices[i] * subSize,
                       subSize * sizeof(T), cudaMemcpyDeviceToDevice));
    }
    ...
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        ...
        &One, V2.l_data() + offset * V2.l_ld(), V2.l_ld(), d_sorted_W, subSize,
 ...
 */
template <typename MatrixType, typename InputMultiVectorType>
void cuda_mpi::pseudo_hermitian_rayleighRitz_v2(
    cublasHandle_t cublas_handle, cusolverDnHandle_t cusolver_handle,
    cusolverDnParams_t params, MatrixType& H, InputMultiVectorType& V1,
    InputMultiVectorType& V2,
    typename ResultMultiVectorType<MatrixType, InputMultiVectorType>::type& W1,
    ...
    chase::distMatrix::RedundantMatrix<
        ...
        ritzv,
    std::size_t offset, std::size_t subSize, int* devInfo,
    typename MatrixType::value_type* workspace, int lwork,
    chase::distMatrix::RedundantMatrix<typename MatrixType::value_type,
                                       chase::platform::GPU>* A)
{
    using T = typename MatrixType::value_type;
    ...
    if (A == nullptr)
    {
        ...
        A_ptr = std::make_unique<
            chase::distMatrix::RedundantMatrix<T, chase::platform::GPU>>(
            2 * subSize, subSize, V1.getMpiGrid_shared_ptr());
        A = A_ptr.get();
    }
    ...
    if (workspace == nullptr)
    {
        ...
        CHECK_CUSOLVER_ERROR(
            chase::linalg::cusolverpp::cusolverDnTheevd_bufferSize(
                ...
                ritzv.l_data() + offset, &lwork));

        int lwork_potrf = 0;
        CHECK_CUSOLVER_ERROR(
            chase::linalg::cusolverpp::cusolverDnTpotrf_bufferSize(
                cusolver_handle, CUBLAS_FILL_MODE_LOWER, subSize, A->l_data(),
                subSize, &lwork_potrf));
    ...
    chase::linalg::internal::cuda_mpi::flipLowerHalfMatrixSign(V1, offset,
                                                               subSize);
    ...
    else
    {
        ...
        CHECK_CUSOLVER_ERROR(chase::linalg::cusolverpp::cusolverDnTheevd(
            cusolver_handle, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_LOWER,
            subSize, M, subSize, ritzv.l_data() + offset, workspace, lwork,
            devInfo));
#ifdef RR_DOUBLE_PRECISION
    }
#endif
