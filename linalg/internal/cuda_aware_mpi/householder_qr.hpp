    ...
    for (std::size_t jj = 0; jj < jb; ++jj)
    {
        ...
        if (n_panel_trail > 0)
        {
            ...
            MPI_Allreduce(
                MPI_IN_PLACE,
                d_w,
                n_panel_trail, chase::mpi::getMPI_Type<T>(),
                MPI_SUM, mpi_col_comm);
            CHECK_CUDA_ERROR(cudaDeviceSynchronize());
            if (vr > 0)
            {
                                         ...
                                         MPI_Comm mpi_col_comm)
{
    ...
    int rank = 0;
    MPI_Comm_rank(mpi_col_comm, &rank);
    ...
    for (std::size_t jj = 0; jj < n; ++jj)
    {
        ...
        MPI_Allreduce(
            MPI_IN_PLACE,
            d_w,
            n_cols, chase::mpi::getMPI_Type<T>(),
            MPI_SUM, mpi_col_comm);
                                                 ...
                                                 cusolverDnHandle_t /* cusolver_handle */)
{
    ...
    for (std::size_t b = 0; b < num_blocks; ++b)
    {
        ...
        if (n_trail > 0)
        {
            ...
            MPI_Allreduce(
                MPI_IN_PLACE,
                d_W,
                jb * jb_next, chase::mpi::getMPI_Type<T>(),
                MPI_SUM, mpi_col_comm);
            ...
            if (n_rest > 0)
            {
                ...
                MPI_Allreduce(
                    MPI_IN_PLACE,
                    d_W,
                    jb * n_rest, chase::mpi::getMPI_Type<T>(),
                    MPI_SUM, mpi_col_comm);
    ...
    for (std::size_t bb = 0; bb < num_blocks; ++bb)
    {
        ...
        MPI_Allreduce(
            MPI_IN_PLACE,
            d_W,
            jb * n_cols, chase::mpi::getMPI_Type<T>(),
            MPI_SUM, mpi_col_comm);
