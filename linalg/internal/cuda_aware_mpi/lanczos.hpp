...
void cuda_mpi::lanczos_dispatch(
    ...
    if constexpr (std::is_same<
                  ...
                  std::is_same<
                      ...
                      chase::distMatrix::PseudoHermitianBlockCyclicMatrix<
                          T, chase::platform::GPU>>::value)
    {
        cuda_mpi::pseudo_hermitian_lanczos(cublas_handle, M, numvec, H, V,
                                           upperb, ritzv, Tau, ritzV);
    }
    else
    {
        cuda_mpi::lanczos(cublas_handle, M, numvec, H, V, upperb, ritzv, Tau,
                          ritzV);
    }
}
    ...
    if constexpr (std::is_same<
                  ...
                  std::is_same<
                      ...
                      chase::distMatrix::PseudoHermitianBlockCyclicMatrix<
                          T, chase::platform::GPU>>::value)
    {
        cuda_mpi::pseudo_hermitian_lanczos(cublas_handle, M, H, V, upperb);
    }
    else
    {
        cuda_mpi::lanczos(cublas_handle, M, H, V, upperb);
    }
}

template <typename MatrixType, typename InputMultiVectorType>
void cuda_mpi::lanczos(cublasHandle_t cublas_handle, std::size_t M,
                       ...
                       chase::Base<typename MatrixType::value_type>* upperb)
{
    ...
    // real_alpha = chase::linalg::blaspp::t_norm_p2(v_1.l_rows(),
    // v_1.l_data());
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTnrm2(
