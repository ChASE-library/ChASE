...
namespace internal
{
     ...
     * work on a multi-GPU setup where MPI is used for communication between GPUs.
     * 
     ...
     * 
     * This function allows for Cholesky QR decomposition, and it supports multi-GPU setups with MPI
    ...
    * handling, including support for communication across devices using CUDA-Aware MPI. 
    * The function can be used in distributed GPU environments and enables
    ...
    * @param ldv The leading dimension of the matrix `V`.
    * @param comm The MPI communicator used for distributed computation across multiple devices.
    ...
    * 
    * @warning This function requires CUDA-Aware MPI for distributed computing. Ensure it  
    ...
    * in parallel across multiple devices using cuBLAS, cuSolver, and CUDA-Aware MPI for distributed 
    * communication. The matrix is processed in panels, allowing for efficient memory management 
    * and computation. The function can be used in a multi-GPU setting with CUDA-Aware MPI. 
    * 
    ...
    *       The decomposition is carried out in a distributed manner with the help of CUDA-Aware MPI 
    ...
    * 
    * @note The function uses CUDA-Aware MPI to perform all-reduce operations across devices to ensure 
    *       that the results are synchronized across multiple GPUs.
    * 
    * @warning This function requires CUDA-Aware MPI for distributed computing. Ensure it 
    *          is properly initialized and that multiple devices are available 
    ...
    *          Ensure that cuBLAS, cuSolver, and CUDA-Aware MPI are properly initialized and that 
    ...
    */
                ...
                T *A)
    {
        ...
        for(auto j = 1; j < number_of_panels; ++j )
        {
            ...
            MPI_Allreduce(MPI_IN_PLACE, 
                        A, 
                        (n - j * panel_size) * panel_size_rest, 
                        chase::mpi::getMPI_Type<T>(),
                        MPI_SUM,
                        comm);    
    ...
    */
    template<typename InputMultiVectorType>
    chase::Base<typename InputMultiVectorType::value_type> cuda_mpi::computeConditionNumber(InputMultiVectorType& V)
    {
        using T = typename InputMultiVectorType::value_type;
