...
{
/**
 * @brief Perform the Rayleigh-Ritz procedure to compute eigenvalues and
 * eigenvectors of a matrix.
 *
 * The Rayleigh-Ritz method computes an approximation to the eigenvalues and
 * eigenvectors of a matrix by projecting the matrix onto a subspace defined by
 * a set of vectors (Q) and solving the eigenvalue problem for the reduced
 * matrix. The computed Ritz values are stored in the `ritzv` array, and the
 ...
 * @param[in] Q The input matrix of size (N x n), whose columns are the basis
 * vectors for the subspace.
 * @param[in] ldq The leading dimension of the matrix Q.
 * @param[out] W The output matrix (N x n), which will store the result of the
 * projection.
 * @param[in] ldw The leading dimension of the matrix W.
 * @param[out] ritzv The array of Ritz values, which contains the eigenvalue
 * approximations.
 * @param[in] A A temporary matrix used in intermediate calculations. If not
 ...
 * 3. Solves the eigenvalue problem for A using LAPACK's `heevd` function,
 * computing the Ritz values in `ritzv`.
 * 4. Computes the final approximation to the eigenvectors by multiplying Q with
...
void rayleighRitz(
    ...
    T* workspace = nullptr, int lwork_heevd = 0,
    chase::matrix::Matrix<T, chase::platform::GPU>* A = nullptr)
{
    ...
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_C, CUBLAS_OP_N, H->rows(), subSize, H->cols(),
        &One, H->data(), H->ld(), V1.data() + offset * V1.ld(), V1.ld(), &Zero,
        V2.data() + offset * V2.ld(), V2.ld()));

    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_C, CUBLAS_OP_N, subSize, subSize, V2.rows(),
        &One, V2.data() + offset * V2.ld(), V2.ld(),
        V1.data() + offset * V1.ld(), V1.ld(), &Zero, A->data(), subSize));
    ...
    {  
        ...
        CHECK_CUSOLVER_ERROR(chase::linalg::cusolverpp::cusolverDnTheevd(
            ...
            subSize, A_d->data(), subSize,
            ritzv_d->data() + offset, workspace_d, lwork_heevd, devInfo));
...
/**
 * @brief Perform the Rayleigh-Ritz procedure to compute eigenvalues and
 * eigenvectors of a Pseudo-Hermitian matrix.
 *
 * The Rayleigh-Ritz method computes an approximation to the eigenvalues and
 * eigenvectors of a matrix by projecting the matrix onto a subspace defined by
 * a set of vectors (Q) and solving the eigenvalue problem for the reduced
...
void rayleighRitz(
    ...
    chase::matrix::Matrix<chase::Base<T>, chase::platform::GPU>& ritzv,
    std::size_t offset, std::size_t subSize, int* devInfo,
    T* d_workspace = nullptr, int d_lwork = 0, T* h_workspace = nullptr,
    int h_lwork = 0,
    chase::matrix::Matrix<T, chase::platform::GPU>* A = nullptr)
{
    SCOPED_NVTX_RANGE();
        ...
        h_workspace == nullptr) // To update once Xgeev is plugged in
    {
        ...
        CHECK_CUSOLVER_ERROR(
            chase::linalg::cusolverpp::cusolverDnTgeev_bufferSize(
                ...
                CUSOLVER_EIG_MODE_VECTOR, n, A->data(), n, ritzv_complex, NULL,
                1, V1.data() + offset * V1.ld(), V1.ld(), &temp_d_lwork,
                &temp_h_lwork));
        if (d_workspace == nullptr)
        {
    ...
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        ...
        V1.data() + offset * V1.ld(), V1.ld(), &Zero,
        V2.data() + offset * V2.ld(),
        V2.ld())); // T = AQr

    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_C, CUBLAS_OP_N, n, n, N, &One,
        V1.data() + offset * V1.ld(), V1.ld(), V2.data() + offset * V2.ld(),
        V2.ld(), &Zero, W, n)); // A = Qr^* T

    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_C, CUBLAS_OP_N, n, n, k, &NegativeTwo,
        V1.data() + offset * V1.ld() + k, V1.ld(),
        V1.data() + offset * V1.ld() + k, V1.ld(), &Zero, M,
        n)); // M = -2 Qr_2^* Qr_2

    chase::linalg::internal::cuda::chase_plus_inverse_diagonal(
        M, n, n, real_One, diag.data(), usedStream);

    chase::linalg::internal::cuda::chase_set_diagonal(M, n, n, Zero,
                                                      usedStream);

    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, n, n, n, &NegOne, M, n, W, n,
        &Zero, A->data(), n)); // A = (Diag(M) - M) * A
    ...
    chase::linalg::internal::cuda::chase_scale_rows_matrix(
        A->data(), n, n, n, diag.data(), usedStream);
    ...
    // std::cout << "Compute the eigenpairs of the non-hermitian rayleigh
    // quotient" << std::endl;
    CHECK_CUSOLVER_ERROR(chase::linalg::cusolverpp::cusolverDnTgeev(
        cusolver_handle, params, CUSOLVER_EIG_MODE_NOVECTOR,
        CUSOLVER_EIG_MODE_VECTOR, n, A->data(), n, ritzv_complex, NULL, 1, W, n,
        d_workspace, d_lwork, h_workspace, h_lwork, devInfo));
    ...
    // thrust::device_vector<int> indices(n); //Does not compile, returns
    // unimplemented on this system...
    if constexpr (std::is_same<T, std::complex<chase::Base<T>>>::value)
    {
        cudaMemcpy(ritzvs_cmplex_cpu.data(), ritzv_complex, n * sizeof(T),
...
#else
#ifdef CHASE_OUTPUT
    std::cout << "WARNING! XGeev not found in cuda. Compute Geev on CPU with "
                 "Lapack..."
              << std::endl;
#endif
    std::vector<chase::Base<T>> ptx_imag =
        std::vector<chase::Base<T>>(n, chase::Base<T>(0.0));
    ...
    lapackpp::t_geev(LAPACK_COL_MAJOR, 'V', n, A->cpu_data(), n, ptx,
                     ptx_imag.data(), W_cpu, n);
    ...
    for (std::size_t i = 0; i < n; ++i)
    {
        ...
        CHECK_CUDA_ERROR(cudaMemcpy(d_sorted_W + i * n, W + indices[i] * n,
    ...
    CHECK_CUBLAS_ERROR(chase::linalg::cublaspp::cublasTgemm(
        cublas_handle, CUBLAS_OP_N, CUBLAS_OP_N, V2.rows(), n, n, &One,
        V1.data() + offset * V1.ld(), V1.ld(), d_sorted_W, n, &Zero,
        V2.data() + offset * V2.ld(), V2.ld()));
}
    ...
    chase::matrix::Matrix<T, chase::platform::GPU>* A = nullptr)
{
    ...
    if (workspace == nullptr) // To update once Xgeev is plugged in
    {
        ...
        CHECK_CUSOLVER_ERROR(
            chase::linalg::cusolverpp::cusolverDnTpotrf_bufferSize(
                cusolver_handle, CUBLAS_FILL_MODE_LOWER, n, A->data(), n,
                &lwork_potrf));
    ...
    {  
        if (A->isDoublePrecisionEnabled())
        {
            A->copyToSubBlockAsync(n * n, subSize * subSize,
                                   &compute_stream);
        }
        else
        {
        ...
        auto A_d = A->getDoublePrecisionMatrix();
        auto ritzv_d = ritzv.getDoublePrecisionMatrix();
        typename ToDoublePrecisionTrait<T>::Type *M_d = A_d->data() + n * n;
        typename chase::ToDoublePrecisionTrait<T>::Type* workspace_d;
        CHECK_CUDA_ERROR(cudaMallocAsync((void**)&workspace_d,
                                         sizeof(typename chase::ToDoublePrecisionTrait<T>::Type) * lwork,
                                         compute_stream));        
        CHECK_CUSOLVER_ERROR(chase::linalg::cusolverpp::cusolverDnTheevd(
            cusolver_handle, CUSOLVER_EIG_MODE_VECTOR, CUBLAS_FILL_MODE_LOWER, n, M_d, n,
            ritzv_d->data() + offset, workspace_d, lwork, devInfo));
        ritzv.disableDoublePrecisionAsync(true, &compute_stream);
        A->copyBackSubBlockAsync(n * n, subSize * subSize,
                                 &compute_stream);
        CHECK_CUDA_ERROR(cudaFreeAsync(workspace_d, compute_stream));
    }
    else
