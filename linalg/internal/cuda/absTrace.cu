                                 ...
                                 std::size_t ld)
{
    ...
    partial_trace[threadIdx.x] =
        (idx < n) ? fabs(d_matrix[idx * ld + idx]) : 0.0f;
                                 ...
                                 std::size_t n, std::size_t ld)
{
    ...
    std::size_t idx = threadIdx.x + blockIdx.x * blockDim.x;
    partial_trace[threadIdx.x] =
        (idx < n) ? fabs(d_matrix[idx * ld + idx]) : 0.0;
    cuda_sum_reduce<NB_X>(threadIdx.x, partial_trace);
    if (threadIdx.x == 0)
    {
        atomicAdd(d_trace, partial_trace[0]);
                                 ...
                                 std::size_t n, std::size_t ld)
{
    ...
    std::size_t idx = threadIdx.x + blockIdx.x * blockDim.x;
    partial_trace[threadIdx.x] =
        (idx < n) ? cuCabsf(d_matrix[idx * ld + idx]) : 0.0f;
    cuda_sum_reduce<NB_X>(threadIdx.x, partial_trace);
    if (threadIdx.x == 0)
    {
        atomicAdd(d_trace, partial_trace[0]);
                                 ...
                                 std::size_t n, std::size_t ld)
{
    ...
    std::size_t idx = threadIdx.x + blockIdx.x * blockDim.x;
    partial_trace[threadIdx.x] =
        (idx < n) ? cuCabs(d_matrix[idx * ld + idx]) : 0.0;
    cuda_sum_reduce<NB_X>(threadIdx.x, partial_trace);
    if (threadIdx.x == 0)
    {
        atomicAdd(d_trace, partial_trace[0]);
                  ...
                  std::size_t ld, cudaStream_t stream_)
{
    ...
    c_absTraceKernel<<<grid, threads, 0, stream_>>>(
        reinterpret_cast<cuComplex*>(d_matrix), d_trace, n, ld);
}
                  ...
                  std::size_t n, std::size_t ld, cudaStream_t stream_)
{
    ...
    z_absTraceKernel<<<grid, threads, 0, stream_>>>(
        reinterpret_cast<cuDoubleComplex*>(d_matrix), d_trace, n, ld);
}
