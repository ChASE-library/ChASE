 ...
 * @brief The ` chase::linalg::blaspp` namespace contains a templated C++
 * interface to the BLAS library.
 *
 * This namespace contains templated functions that interface with the BLAS
 * library to provide efficient linear algebra routines. The functions are
 * templated to work with different data types such as `float`, `double`, and
 * `std::complex`. These functions allow for easy and efficient numerical
 ...
 *
 * This function computes the square root of the L2 norm (Euclidean norm) of a
 * vector `x`. It is equivalent to the `BLAS` function `nrm2`.
 *
 * @tparam T The data type of the vector elements. Supported types: `float`,
 ...
 *
 * This function computes the Euclidean distrance of a vector `x`, which is the
 ...
 *
 * This function computes the L2 norm (Euclidean norm) of a vector `x`. It is
 * equivalent to the `BLAS` function `nrm2`.
 *
 * @tparam T The data type of the vector elements. Supported types: `float`,
 * `double`, `std::complex<float>`, `std::complex<double>`.
 *
 ...
 *
 * This function computes the dot product of two vectors `x` and `y`. It is
 * equivalent to the `BLAS` function `dot`.
 *
 * @tparam T The data type of the vector elements. Supported types: `float`,
 * `double`, `std::complex<float>`, `std::complex<double>`.
 *
 ...
 *
 * This function scales a vector `x` by a scalar `a`. It is equivalent to the
 * `BLAS` function `scal`.
 *
 * @tparam T The data type of the vector elements. Supported types: `float`,
 * `double`, `std::complex<float>`, `std::complex<double>`.
 ...
 *
 * This function performs a rank-1 update of vector `y` by adding the scaled
 * vector `a * x`. It is equivalent to the `BLAS` function `axpy`.
 *
 * @tparam T The data type of the vector elements. Supported types: `float`,
 * `double`, `std::complex<float>`, `std::complex<double>`.
 *
 ...
 *
 * This function performs the symmetric rank-k update of the form `A = alpha * x
 * * x' + beta * A`. It is equivalent to the `BLAS` function `syherk`.
 *
 * @tparam T The data type of the matrix elements. Supported types: `float`,
 * `double`, `std::complex<float>`, `std::complex<double>`.
 *
 * @param uplo Specifies which part of the matrix to update ('U' for upper, 'L'
 * for lower).
 * @param trans Specifies the type of transpose ('N' for no transpose, 'T' for
 * transpose, 'C' for conjugate transpose).
 ...
 *
 * This function solves a triangular system `A * X = B` or `X * A = B`, where
 * `A` is a triangular matrix. It is equivalent to the `BLAS` function `trsm`.
 *
 * @tparam T The data type of the matrix elements. Supported types: `float`,
 * `double`, `std::complex<float>`, `std::complex<double>`.
 *
 * @param side Specifies whether `A` is on the left or right side of the
 * equation.
 ...
 *
 * This function performs the matrix multiplication `C = alpha * A * B + beta *
 * C`. It is equivalent to the `BLAS` function `gemm`.
 *
 * @tparam T The data type of the matrix elements. Supported types: `float`,
 * `double`, `std::complex<float>`, `std::complex<double>`.
 *
