...
public:
    ChaseElemental(std::size_t N, std::size_t nev, std::size_t nex,
                   El::DistMatrix<T>& H)
        : N_(N), nev_(nev), nex_(nex), locked_(0), config_(N, nev, nex), H_(H),
          V_(N_, nev_ + nex_, H_.Grid()), W_(N_, nev_ + nex_, H_.Grid()),
          // ritzv_(H_.Grid()),
          ritzv_(nev_ + nex_), resid_(nev_ + nex_), approxV_(&V_),
          workspace_(&W_)
    {
        El::MakeGaussian(V_);
        El::MakeGaussian(W_);
    ...
    void QR(std::size_t fixednev) override
    {
        ...
        auto saved_locked_vectors =
            El::LockedView(*workspace_, 0, 0, N_, locked_);
    ...
    void RR(Base<T>* ritzv, std::size_t block) override
    {
        ...
        El::HermitianEig(El::LOWER, H_reduced, ritzv_tmp, V_reduced,
                         El::ASCENDING);
    ...
    void Lanczos(std::size_t m, Base<T>* upperb) override
    {
        ...
        *upperb = std::max(std::abs(lambda.Get(0, 0)),
                           std::abs(lambda.Get(m - 1, 0))) +
    ...
    void LanczosDos(std::size_t idx, std::size_t m, T* ritzVc) override
    {
        ...
        // m));
        // auto dos_vectors = El::View(*approxV_, El::IR(0, N_), El::IR(0,
        // idx));
