...
public:
  ...
  // TODO: everything should be owned by chASE_Blas, deg should be part of ChasE_Config
  void solve() {
  ...
  void threeTerms( CHASE_INT nev, T alpha, T beta, CHASE_INT offset ) {
    t_gemm(
           ...
           N, nev, N,
           &alpha, H, N, approxV+locked*N+offset, N,
           &beta, workspace+locked*N+offset, N );
  ...
  void resd( Base<T> *ritzv, Base<T> *resid, CHASE_INT fixednev )
  {
    ...
    for( std::size_t i = 0 ; i < unconverged ; ++i )
      {
        beta  = T(-ritzv[i], 0.0);
        t_axpy( N, &beta, (approxV+locked*N)+N*i, 1, (workspace+locked*N)+N*i, 1);
  ...
  void lanczos( CHASE_INT M, Base<T> *upperb, Base<T> *ritzv, Base<T> *Tau, Base<T> *ritzV )
  {
    // todo
    CHASE_INT m = M;
    CHASE_INT n = N;
  ...
  {
    memcpy( workspace+locked*N, approxV+locked*N, N*(new_converged)*sizeof(T) );
		  ...
		  const double tol, std::size_t deg )
{
  ...
  if(deg < 8 || deg > ChASE_Config::chase_max_deg)
    std::clog << "WARNING: Degree should be between 8 and " << ChASE_Config::chase_max_deg << "."
