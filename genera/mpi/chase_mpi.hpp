...
public:
    ////////////////////////////////////////////////////////////////////////////
    // TODO think up something for the cascading initializers
    ////////////////////////////////////////////////////////////////////////////
    /*
    ChASE_MPI(ChASE_Config _config, MPI_Comm aComm, Base<T>* ritzv_, T* H_, T* V_, T*W_ )
        : N(_config.getN())
        , nev(_config.getNev())
        , nex(_config.getNex())
        , locked(0)
    ...
    void doGemm(T alpha, T beta, CHASE_INT offset, CHASE_INT nev)
    {
        ...
        if (next == 'b') {
            ...
            t_gemm(CblasColMajor, CblasNoTrans, CblasNoTrans,
                m, static_cast<std::size_t>(nev), n,
                &One,
                H, m,
                B + offset * n, n,
                &Zero,
        ...
        */
            ...
            MPI_Allreduce(MPI_IN_PLACE, IMT + offset * m,
                dim, getMPI_Type<T>(), MPI_SUM, rowComm);

            t_scal(dim, &beta, C + offset * m, 1);
            t_axpy(dim, &alpha, IMT + offset * m, 1,
                C + offset * m, 1);
        ...
        if (next == 'c') {
            ...
            t_gemm<T>(CblasColMajor, CblasConjTrans, CblasNoTrans,
                n, static_cast<std::size_t>(nev), m,
                &One,
                H, m,
                C + offset * m, m,
        ...
        */
            ...
            MPI_Allreduce(MPI_IN_PLACE, IMT + offset * n,
                dim, getMPI_Type<T>(), MPI_SUM, colComm);

            t_scal(dim, &beta, B + offset * n, 1);
            t_axpy(dim, &alpha, IMT + offset * n, 1,
                B + offset * n, 1);
