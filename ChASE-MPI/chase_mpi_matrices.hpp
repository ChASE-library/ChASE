 ...
 */
//! A class to setup the buffers of matrices and vectors which will be used by ChASE.
/*!
...
class ChaseMpiMatrices {
 public:
  ...
  /*!
    ...
    @param max_block: Maximum column number of matrix `V1_` and `V2_`. It equal to `nev_ + nex_`.
  ...
  */
  ...
  /*!
    ...
    For `V1__`, V1_`, V2__` and `V1_`, they are of size `N * max_block`.
    For `ritzv__`, ritzv_`, resid__` and `resid_`, they are of size `N * max_block`.
    `H_` is not allocated here, but in ChaseMpiProperties class.
    @param comm: the working MPI communicator to ChASE.
    @param N: size of the square matrix defining the eigenproblem.
    @param max_block: Maximum column number of matrix `V1_` and `V2_`. It equal to `nev_ + nex_`.
    @param V1: a pointer to the buffer `V1_`.
    @param ritz: a pointer to the buffer `ritz_`.
  ...
  */        
  ...
  T* get_V1() { return V1_; }
  // Return the buffer `V2_` whose conjugate transpose  which will be left-multiplied to `A` during the process of ChASE.
 ...
 private:
  //! A smart pointer which manages the buffer of `H_` which stores the (local part if applicable) matrix `A`. 
  std::unique_ptr<T[]> H__;
  //! A smart pointer which manages the buffer of `V1_` which  which will be right-multiplied to `A` during the process of ChASE. The eigenvectors obtained will also stored in `V1_`.  
  std::unique_ptr<T[]> V1__;
  //! A smart pointer which manages the buffer of `V2_` whose conjugate transpose  which will be left-multiplied to `A` during the process of ChASE.
  ...
  //! The buffer stores the (local part if applicable) matrix `A`.
  T* H_;
  //! The buffer of `V1_`  which will be right-multiplied to `A` during the process of ChASE. The eigenvectors obtained will also stored in `V_`.
  T* V1_;
  //! The buffer of `V2_` whose conjugate transpose  which will be left-multiplied to `A` during the process of ChASE.
  T* V2_;
