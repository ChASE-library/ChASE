...
class ChaseMpiDLABlaslapackSeqInplace : public ChaseMpiDLAInterface<T> {
  ...
  */
  void apply(T alpha, T beta, std::size_t offset, std::size_t block,  std::size_t locked) override {
      t_gemm(CblasColMajor, CblasNoTrans, CblasNoTrans, N_,
                ...
                V1_ + offset * N_ + locked * N_, N_, &beta, V2_ + locked * N_ + offset * N_, N_);    
  ...
  */
  ...
  /*!
    - For ChaseMpiDLABlaslapackSeqInplace, `gemm` is implemented using `BLAS` routine `xGEMM`.
    - **Parallelism is SUPPORT within node if multi-threading is actived**    
    - For the meaning of this function, please visit ChaseMpiDLAInterface.
  */
  ...
  */
  ...
  /*!
      - For ChaseMpiDLABlaslapackSeqInplace, `RR` is implemented by `GEMM` routine provided by `BLAS` and `(SY)HEEVD` routine provided by `LAPACK`.
  ...
  */  
  ...
  void cholQR(std::size_t locked) override{
    ...
    if(info == 0){
      t_trsm('R', 'U', 'N', 'N', N_, nevex, &one, A_.get(), nevex, V1_, N_);
