...
/**
 * This class provides the basic functionality for a series of  a multi-GPU
 * Hemm-s. The core functionalies are to distribute H, V and W between the GPU
 * devices and to perform the Hemm operation in distributed manner among all GPU
 * devices. The class supports two type of operations
 *
 * 		W = alpha * H * V + beta * W (refered as 'cAb' operation)
 * or
 * 		V = alpha * H^C * W + beta * V (refered as 'bAc' operation)
 *
 * 	such that not additional redististribution of H is required between
 * successive Hemm calls.
 */
...
/*! This class mainly implements a multi-GPU for `HEMM`, which is able to be
executed either with 1 MPI managing multi-GPUs or each GPU being bounded to one
MPI rank. This class mainly provides the basic functionality for a series of  a
multi-GPU Hemm-s. The core functionalies are to distribute H, V and W between
the GPU devices and to perform the Hemm operation in distributed manner among
all GPU devices. The class supports two type of operations
    - W = alpha * H * V + beta * W (refered as 'cAb' operation)
    - V = alpha * H^C * W + beta * V (refered as 'bAc' operation)
...
public:
    ...
    /* Constructor - sets GPUs, allocate device arrays, create cublas handles
     * and streams */
    mgpu_cudaDLA(){};

    /* Constructor - sets GPUs, allocate device arrays, create cublas handles
     * and streams */
    //! A constructor of mgpu_cudaDLA which sets GPUs, allocate device arrays,
    //! create cublas and cusolver handles and streams.
    /*!
            @param matrix_properties: 	it is an object of ChaseMpiProperties,
       which defines the MPI environment and data distribution scheme in
       ChASE-MPI.
            @param m: row number of local matrix stored on each MPI rank.
            @param n: column number of local matrix stored on each MPI rank.
            @param maxBlock: maximum number of column of the rectangular matrix
       `V`, which equals to `nev+nex`.
    */
    mgpu_cudaDLA(ChaseMpiProperties<T>* matrix_properties, std::size_t m,
                 std::size_t n, std::size_t maxBlock)
        : m_(m), n_(n), maxBlock_(maxBlock)
    {
        ...
        handle_ = (cublasHandle_t*)malloc(num_devices_per_rank *
                                          sizeof(cublasHandle_t));
        stream_ =
            (cudaStream_t*)malloc(num_devices_per_rank * sizeof(cudaStream_t));

        cusolverH_ = (cusolverDnHandle_t*)malloc(num_devices_per_rank *
        ...
        /* Allocate arrays to hold pointers to memory allocations on each device
         * for matrices H, B and IMT */
        ...
        /* Pointers to memory location of the HEMM operands. The class always
         * computes W = H * V + W operation */
        ...
        pitchB =
            (std::size_t*)malloc(num_devices_per_rank * sizeof(std::size_t));
        pitchH =
            (std::size_t*)malloc(num_devices_per_rank * sizeof(std::size_t));
        pitchIMT =
            (std::size_t*)malloc(num_devices_per_rank * sizeof(std::size_t));
        ...
        /// TODO: ldB and ldIMT does not to be large as ldWRK but rather
        /// dim_tile_m and dim_tile_n_, respectively
        ...
        for (int dev_x = 0; dev_x < ntile_m_; dev_x++)
        {
            ...
            for (int dev_y = 0; dev_y < ntile_n_; dev_y++)
            {
                ...
                /* Set device */
                cuda_exec(
                    cudaSetDevice(shmrank_ * num_devices_per_rank + gpu_id));

                //* Allocate memories for H, IMT and B matrices */
                cuda_exec(cudaMallocPitch((void**)&B_[gpu_id], &pitchB[gpu_id],
                                          maxBlock_ * sizeof(T), ldB));
                cuda_exec(cudaMallocPitch((void**)&IMT_[gpu_id],
                                          &pitchIMT[gpu_id],
                                          maxBlock_ * sizeof(T), ldIMT));
                cuda_exec(cudaMallocPitch((void**)&WRKSPACE_[gpu_id],
                                          &pitchWRK[gpu_id],
                                          maxBlock_ * sizeof(T), ldWRK));
                cuda_exec(cudaMallocPitch((void**)&H_[gpu_id], &pitchH[gpu_id],
                                          tile_y * sizeof(T), ldH));
            }
        }
        ...
        for (int dev_x = 0; dev_x < ntile_m_; dev_x++)
        {
            ...
            for (int dev_y = 0; dev_y < ntile_n_; dev_y++)
            {
                    ...
                    for (std::size_t i = 0; i < mblocks_; i++)
                    {
                        for (std::size_t q = 0; q < c_lens_[j]; q++)
                        {
                            for (std::size_t p = 0; p < r_lens_[i]; p++)
                            {
                                ...
                                if (q + c_offs_l_[j] >= start_col &&
                                    q + c_offs_l_[j] < start_col + tile_y &&
                                    p + r_offs_l_[i] >= start_row &&
                                    p + r_offs_l_[i] < start_row + tile_x)
                                {
                                    std::size_t s, t;
                                    // t, s, global index
                                    t = q + c_offs_[j];
                ...
                cuda_exec(
                    cudaSetDevice(shmrank_ * num_devices_per_rank + dev_id));
                cudaMalloc(&d_off_m_[dev_id], off_size * sizeof(std::size_t));
                cudaMalloc(&d_off_n_[dev_id], off_size * sizeof(std::size_t));
                cudaMemcpy(d_off_m_[dev_id], off_m.data(),
                           off_size * sizeof(std::size_t),
                           cudaMemcpyHostToDevice);
                cudaMemcpy(d_off_n_[dev_id], off_n.data(),
                           off_size * sizeof(std::size_t),
                           cudaMemcpyHostToDevice);
            }
        }
        ...
        //
        /* Keep info wether the matrix H_ is distributed among devices or not.
         * In the initialization phase, it is not distributed */
        copied_ = false;
        next_ = NextOp::cAb;
    ...
    ~mgpu_cudaDLA()
    {
        ...
        for (int dev = 0; dev < num_devices_per_rank; dev++)
        {
            ...
            if (cusolverH_[dev])
                cusolverDnDestroy(cusolverH_[dev]);
    ...
    */
    ...
    void distribute_H(T* orig_H, std::size_t ld_origH)
    {
        ...
        /* If H is not distributed among devices (i.e. the first call to the
         * distribute_H), distribute it */
        if (!copied_)
        {
            ...
            for (int dev_x = 0; dev_x < ntile_m_; dev_x++)
            {
                ...
                for (int dev_y = 0; dev_y < ntile_n_; dev_y++)
                {
                    ...
                    /* Set a device and asyncronously transfer the tile to that
                     * device's memory */
                    cuda_exec(cudaSetDevice(shmrank_ * num_devices_per_rank +
                                            dev_id));

                    cublas_exec(cublasSetMatrixAsync(
                        tile_x, tile_y, sizeof(T),
                        &orig_H[start_col * ld_origH + start_row], ld_origH,
                        H_[dev_id], ldH, stream_[dev_id]));
                }
            }
    ...
    void shiftMatrix(T c, bool isunshift = false)
    {
        ...
        for (int dev_x = 0; dev_x < ntile_m_; dev_x++)
        {
            ...
            for (int dev_y = 0; dev_y < ntile_n_; dev_y++)
            {
                ...
                cuda_exec(
                    cudaSetDevice(shmrank_ * num_devices_per_rank + dev_id));
                chase_shift_mgpu_matrix(H_[dev_id], d_off_m_[dev_id],
                                        d_off_n_[dev_id], off_size, ldH,
                                        std::real(c), stream_[dev_id]);
            }
        }
    }

    /* Divide given matrix buf_init into panels and distributed among GPUs */
    //! This member function divides given matrix `buf_init` into panels and
    //! distributed among GPUs
    /*!
            @param buf_init: the given matrix to be divided into panels and
       distributed among GPUs
            @param ldBuf: the leading dimension of `buf_init`
            @param block: number of non-converged eigenvectors, it indicates the
       number of columns in `V` which performs the `HEMM` operation.
    */
    void distribute_V(T* buf_init, std::size_t ldBuf, std::size_t block)
    {
        ...
        if (next_ == NextOp::cAb)
        {
            ...
            /* Divide buf_init in row-panels matching the number of tile-columns
             * of tiled H */
            for (int i = 0; i < ntile_n_; i++)
            {
                ...
                for (int dev = i; dev < num_devices_per_rank; dev += ntile_n_)
                {
                    ...
                    cuda_exec(
                        cudaSetDevice(shmrank_ * num_devices_per_rank + dev));
                    cublas_exec(cublasSetMatrixAsync(
                        tile_x, block, sizeof(T), &buf_init[start_row], ldBuf,
                        V[dev], ldV, stream_[dev]));
                }
            }
        }
        /* The case V = H^C * W + V */
        ...
        {
            /* Divide buf_init input tile ntile_m_ tiles and distribute to GPU
             * such as that tile i goes to the GPUs with GPU_ID % ntile_m_ == i
             */
            /* Divide buf_init in row-panels matching the number of tile-rows of
             * the tiled H (because H is (conjugate) transposed */
            for (int i = 0; i < ntile_m_; i++)
            {
                ...
                /* Compute index of the first GPU device in the (transponsed)
                 * tile-row of the tiled matrix H */
                int start_dev_id = i * ntile_n_;

                /* Pass through all the devices in the tile-row */
                for (int dev = start_dev_id; dev < start_dev_id + ntile_n_;
                     dev++)
                {
    ...
    */
    void computeHemm(std::size_t block, std::size_t offset, T alpha, T beta)
    {
        ...
        /* Shift matrix W. W is holding B or C column-matrix from previous
         * iteration */
        ...
        /* Visit all the tiles of the tile matrix H and compute local (partial)
         * HEMM operations */
        ...
        for (int dev_x = 0; dev_x < ntile_m_; dev_x++)
        {
            ...
            for (int dev_y = 0; dev_y < ntile_n_; dev_y++)
            {
                 ...
                 * The first GPU of each row stored intermediat results in W,
                 * others in the WRKSPACE array */
                if (next_ == NextOp::cAb)
                {
                    m = tile_x;
                    n = block;
                ...
                /* Set device and compute local Tgemm. The GPUs operating on the
                 * the first tiles in the rows are updating
                 * W, while other GPUs store intermediate results in the cleaned
                 * (multiplied by '0') WRKSPACE array */
                cuda_exec(
                    cudaSetDevice(shmrank_ * num_devices_per_rank + dev_id));
                if (leading_gpu)
                {
                    cublas_exec(cublasTgemm(handle_[dev_id], transa,
                                            CUBLAS_OP_N, m, n, k, &alpha,
                                            H_[dev_id], ldH, V[dev_id], ldV,
                                            &beta, W[dev_id], ldW));
                }
                else
                {
         ...
         * Collect intermediate results from the GPUs in the same rows to the
         * first GPU in the row. Implemented as a parallel prefix sum. In the
         * first step the odd GPUs transfer their partial solutions (tile
         * products) to a one previous GPU (i.e. the one with the index-1) where
         * the sum of two tiles are computed and so on. */
        for (int s = 1; s < num_tile_cols; s <<= 1)
        {
            ...
            if (next_ == NextOp::cAb)
            {
                for (int dev = s; dev < num_devices_per_rank; dev += 2 * s)
                {
                    ...
                    if (s == 1 || (num_tile_cols % 2 != 0 &&
                                   dev == num_devices_per_rank - 1))
                    {
                        cuda_exec(cudaMemcpyAsync(
                            WRKSPACE_[dev - s], WRKSPACE_[dev],
                            block * sizeof(T) * ldWRK, cudaMemcpyDeviceToDevice,
                            stream_[dev - s]));
                    }
                    else
                    {
                        cuda_exec(cudaMemcpyAsync(
                            WRKSPACE_[dev - s], W[dev], block * sizeof(T) * ldW,
                            cudaMemcpyDeviceToDevice, stream_[dev - s]));
                    }
                    cuda_exec(cudaSetDevice(shmrank_ * num_devices_per_rank +
                                            dev - s));

                    cublas_exec(cublasTaxpy(handle_[dev - s], block * ldW, &one,
                                            WRKSPACE_[dev - s], 1, W[dev - s],
                                            1));
                }
            }
            else
            {
                ...
                {
                    for (int dev_y = dev_x + s * ntile_n_;
                         dev_y < num_devices_per_rank;
                         dev_y += 2 * s * ntile_n_)
                    {
                        gpu_src = dev_y;
                        ...
                        if (s == 1 || (num_tile_cols % 2 != 0 &&
                                       dev_y / ntile_n_ == ntile_m_ - 1))
                        {
                            cuda_exec(cudaMemcpyAsync(
                                WRKSPACE_[gpu_dest], WRKSPACE_[gpu_src],
                                block * sizeof(T) * ldWRK,
                                cudaMemcpyDeviceToDevice, stream_[gpu_dest]));
                        }
                        else
                        {
                            cuda_exec(cudaMemcpyAsync(
                                WRKSPACE_[gpu_dest], W[gpu_src],
                                block * sizeof(T) * ldW,
                                cudaMemcpyDeviceToDevice, stream_[gpu_dest]));
                        }
                        cuda_exec(cudaSetDevice(
                            shmrank_ * num_devices_per_rank + gpu_dest));

                        cublas_exec(cublasTaxpy(handle_[gpu_dest], block * ldW,
                                                &one, WRKSPACE_[gpu_dest], 1,
                                                W[gpu_dest], 1));
                    }
                }
            }
    ...
    //! This member function collects and returns the computed `W` from the GPUs
    //! to the host
    /*!
            @param buf_target: the matrix host which the computed `W` should be
       copied to.
    ...
    */
                  ...
                  std::size_t offset)
    {
        ...
        if (next_ == NextOp::cAb)
        {
            for (int dev_x = 0; dev_x < ntile_m_; dev_x++)
            {
                ...
                cuda_exec(
                    cudaSetDevice(shmrank_ * num_devices_per_rank + src_gpu));
                cublas_exec(cublasGetMatrixAsync(
                    tile_x, block, sizeof(T), W[src_gpu], ldW,
                    &buf_target[start_row], ldBuf, stream_[src_gpu]));
            }
        }
        else
        {
    ...
    /* Switch pointers to per-device 2D arrays depending on the next operation
     * (cAb or bAc) */
    //! This member function switches pointers to per-device 2D arrays depending
    //! on the next operation (cAb or bAc)
    ...
    //! This function defines a switch operation, between W = H * V + W and V =
    //! H^C W + V
...
private:
    ...
    std::chrono::duration<double, std::milli> time_gemm, time_dist, time_axpy,
        time_redist;
    ...
    /// Return the number of columns of the tile with column-index
    /// 'tile_position'
