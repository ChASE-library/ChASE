...
v    float normH = std::max<float>(t_lange('1', *N, *N, H, *N), float(1.0));
...
void c_zchase_(std::complex<double>* H, int* N, std::complex<double>* V,
               double* ritzv, int* nev, int* nex, int* deg, double* tol,
               char* mode, char* opt) {
  std::cout << "entering chase" << std::endl;
  std::cout << "tol: " << *tol << std::endl;
  ChaseConfig<std::complex<double>> config(*N, *nev, *nex);
  ...
  std::unique_ptr<MatrixFreeChase<std::complex<double>>> single_ =
      constructChASE(config, H, V, ritzv, MPI_COMM_SELF);

  single_->Solve();
}

void c_dchase_(double* H, int* N, double* V, double* ritzv, int* nev, int* nex,
               int* deg, double* tol, char* mode, char* opt) {
  std::cout << "entering chase" << std::endl;
  std::cout << "tol: " << *tol << std::endl;
  ChaseConfig<double> config(*N, *nev, *nex);
  ...
  if (!config.use_approx())
    for (std::size_t k = 0; k < *N * (*nev + *nex); ++k) V[k] = d(gen);

  std::unique_ptr<MatrixFreeChase<double>> single_ =
      constructChASE(config, H, V, ritzv, MPI_COMM_SELF);
