...
std::vector<T> generateEyeMat(std::size_t N)
{
    ...
    if (rank == 0)
        std::cout << "]> Generating an Identity matrix as S" << std::endl;
...
std::vector<T> generateClementMat(std::size_t N)
{
    ...
    MPI_Comm_rank(MPI_COMM_WORLD, &rank);
    if (rank == 0)
        std::cout << "]> Generating a Clement matrix as H" << std::endl;
    ...
    for (auto i = 0; i < N; ++i)
    {
        C[i + N * i] = 0;
        if (i != N - 1)
            C[i + 1 + N * i] = std::sqrt(i * (N + 1 - i));
        if (i != N - 1)
            C[i + N * (i + 1)] = std::sqrt(i * (N + 1 - i));
    }
                ...
                std::size_t* c_offs, std::size_t* c_lens, std::size_t* c_offs_l)
{
    ...
    if (rank == 0)
        std::cout << "]> Loading " << problem.str() << std::endl;
...
int do_chase_gev(ChASE_DriverProblemConfig& conf)
{
    ...
    descinit(desc, &N, &N, &mbsize, &nbsize, &irsrc, &irsrc, &ictxt, &lld_loc,
             &info);
    ...
    CHASE single(new ChaseMpiProperties<T>(N, mbsize, nbsize, nev, nex, dim0,
                                           dim1,
                                           const_cast<char*>(major.c_str()),
                                           irsrc, icsrc, MPI_COMM_WORLD),
                 V, Lambda);
    ...
    for (auto idx = bgn; idx <= end; ++idx)
    {
                    ...
                    for (std::size_t q = 0; q < c_lens[j]; q++)
                    {
                        for (std::size_t p = 0; p < r_lens[i]; p++)
                        {
                            H[(q + c_offs_l[j]) * m + p + r_offs_l[i]] =
                                HH[(q + c_offs[j]) * N + p + r_offs[i]];
                            S[(q + c_offs_l[j]) * m + p + r_offs_l[i]] =
                                SS[(q + c_offs[j]) * N + p + r_offs[i]];
                        }
                    }
                }
            }
        }
        else // if path of matrices are given, load them into H and S by
             // parallel IO
        {
            // read matrix H from local
            readMatrix(H, path_in, "hmat", idx, ".bin", N * N, m, mblocks,
                       nblocks, r_offs, r_lens, r_offs_l, c_offs, c_lens,
                       c_offs_l);
            // read matrix S from local
            readMatrix(S, path_in, "smat", idx, ".bin", N * N, m, mblocks,
                       nblocks, r_offs, r_lens, r_offs_l, c_offs, c_lens,
                       c_offs_l);
        }
        ...
        elapsed[0][idx - 1] =
            std::chrono::duration_cast<std::chrono::duration<double>>(ed -
                                                                      st_loc);
        ...
        // with H' = L^{-1} * H * (L^T)^{-1}
        t_psyhegst<T>(i_one, 'U', N, H, sze_one, sze_one, desc, S, sze_one,
                      sze_one, desc, &scale);

        // ensure H' to be exact Symmetric/Hermtian by H' =
        // 0.5*H'+0.5*transpose(H')
        T* tmpH = new T[m * n];
        std::memcpy(tmpH, H, m * n * sizeof(T));
        t_geadd<T>('T', N, N, alpha, tmpH, sze_one, sze_one, desc, alpha, H,
                   sze_one, sze_one, desc);
        ...
        // In order to recover the generalized eigenvectors by Scalapack, it
        // should be redistributed into block-cyclic format. We re-use H to
        // restore V. this part is in parallel implicitly
        ...
        for (std::size_t j = 0; j < nblocks; j++)
        {
            for (std::size_t i = 0; i < mblocks; i++)
            {
                for (std::size_t q = 0; q < c_lens[j]; q++)
                {
                    for (std::size_t p = 0; p < r_lens[i]; p++)
                    {
                        if ((q + c_offs[j]) * N + p + r_offs[i] <
                            (nev + nex) * N)
                        {
                            H[(q + c_offs_l[j]) * m + p + r_offs_l[i]] =
                                V[(q + c_offs[j]) * N + p + r_offs[i]];
                        }
                    }
                }
            }
        ...
        // Now the first (nev+nex) columns of H (a global view) is overwritten
        // by V Recover the genealized eigenvectors X by solving X' = L^T * X

        t_ptrtrs<T>('U', 'N', 'N', N, nev + nex, S, sze_one, sze_one, desc, H,
                    sze_one, sze_one, desc);
        ...
        elapsed[4][idx - 1] =
            std::chrono::duration_cast<std::chrono::duration<double>>(ed -
                                                                      st_loc);
        elapsed[5][idx - 1] =
            std::chrono::duration_cast<std::chrono::duration<double>>(ed - st);
    ...
    if (rank == 0)
    {
        ...
        std::cout
            << "---------------------------------------------------------------"
               "---------------------------------------------------------------"
               "---------------------------------\n"
            << "|  Index|             Parallel IO|  Cholesky Factorization|    "
               "Transfer to Standard|            ChASE Solver|          Back "
               "Transform|                     All|\n"
            << "|-------|------------------------|------------------------|----"
               "--------------------|------------------------|-----------------"
               "-------|------------------------|";
