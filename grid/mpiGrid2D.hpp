 ...
 * The `chase::grid` namespace defines classes and enumerations for creating and
 * managing a 2D MPI grid, with support for row-major and column-major ordering.
 ...
 * If NCLL is enabled, it will create corresponding NCCL communicators on top of
 * it.
 */
 ...
 * @{
 * @brief Namespace containing classes and enumerations related to MPI 2D grid
 * setup.
 */
 ...
 *
 * @tparam MajorOrder Specifies the major ordering of the grid (RowMajor or
 * ColMajor).
 */
...
public:
     ...
     */
    MpiGrid2D(int row_dim, int col_dim, MPI_Comm comm) : comm_(comm)
    {
        dims_[0] = row_dim;
        dims_[1] = col_dim;
    ...
    /**
     * @brief Constructs a 2D MPI grid with dimensions derived from the
     * communicator.
     *
     * @param comm MPI communicator used to create the grid.
     */
    MpiGrid2D(MPI_Comm comm) : comm_(comm)
    {
        MPI_Comm_size(comm_, &nprocs_);
...
private:
    ...
    void create2DGrid()
    {
        ...
        for (auto i = 0; i < dims_[0]; i++)
        {
            if (coords_[0] == i)
            {
                CHECK_NCCL_ERROR(ncclCommInitRank(&nccl_row_comm_, dims_[1],
                                                  nccl_ids[i], coords_[1]));
            }
        }
